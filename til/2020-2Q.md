...
[대부분의 경우에 안 쓰는 것이 권장](https://stackoverflow.com/questions/5239066/what-can-i-use-instead-of-a-vector-in-java)되는 Vector이긴 하지만 자꾸만 보여서 정리.

- `ArrayList`와 다르게 synchronized.
- 그 만큼 오버헤드가 따름.
- 동기화가 필요하다고 하더라도 `CopyOnWriteArrayList`로 충분할 수 있음.

```
읽기 연산이 동기화를 사용하지 않기 때문. 성능에 유리.
읽기가 동기화여야 하는 경우가 많지 않음.
게다가 iterator가 concurrent modification를 지원.
하지만 이를 위해 매번 컬렉션이 복제됨.
잦은 수정에는 부담인 것.
```

- 한편, 정말 모든 연산이 동기화여야 한다면, `Collections.synchronizedList` 사용을 고려.
- 이는 `Vector`에게 열려 있는 `elements()`(legacy) 사용을 피할 수 있게 도와줌.

```
`Vector#elements`는 `Enumeration`을 반환.
이는 fail-fast semantic을 지원하지 않음.
`ConcurrentModificationException`을 반환하지 않는 것.
좀 더 자세한 내용은 아래 링크 참고.
https://stackoverflow.com/questions/10880137/java-enumeration-vs-iterator
```

- `ArrayList`처럼 동적으로 크기를 늘리지만, `ArrayList`가 현재 크기의 50%만 늘어나는 반면, `Vector`는 100%.
- 위에서 언급했듯이 `Vector`는 `Enumeration`과 `Iterator` 제공.
- `ArrayList`는 fail-fast semantic을 지원하는 `Iterator`만 제공.
...
자바의 [Character 문서](https://docs.oracle.com/javase/6/docs/api/java/lang/Character.html)에 보면, [Unicode Character Representations](https://docs.oracle.com/javase/6/docs/api/java/lang/Character.html#unicode) 부분이 있음. 간단히 기록.

1. `char` 데이터 타입은 오리지널 유니코드 명세에 기반.
2. 이는 글자(character)를 fixed-width 16-bit entities로 정의.
3. 유니코드는 계속 변화했고, 현재는 16비트 이상으로 표현되는 문자를 허용.
4. 이로 인해, *code points*는 현재 U+0000에서 U+10FFFF 까지의 유효 범위를 가짐. *Unicode scalar value*라고 부름.
6. U+0000에서 U+FFFF까지의 문자 집합은 *Basic Multilingual Plane*(BMP)이라 부름.
7. U+FFFF가 넘어가는 코드 포인트의 문자들은 *supplementary characters*라 부름.
8. 이 supplementary characters는 char 값의 쌍으로 표현.
9. 첫 번째 값은 high-surrogates 범위(\uD800-\uDBFF)를 갖고,
10. 두 번째 값은 low-surrogates 범위(\uDC00-\uDFFF)를 가짐.
11. int 값은 모든 유니코드 코드 포인트를 표현할 수 있음.
12. int의 최하위 21개 비트는 유니코드 코드 포인트를 나타내는 데 쓰이고, 나머지 최상위 11개 비트는 0이어야 함.
13. 참고로, UTF16 char을 2개 이용하니 32개 비트를 쓰는 것이고, 21개 비트가 쓰이는 까닭은 위에서 코드 포인트의 최대값이 U+10FFFF 즉, 111411(바이너리 숫자로는 100001111111111111111)이기 때문.
14. char 값만을 받는 메서드는 supplementary characters 지원 불가.
15. 예컨대, `Character.isLetter('\uD840')`은 `false`를 반환.
16. 하지만 `\uD840`는 문자열에서는 글자(letter) 표현에 사용.
17. 한편,`isLetter(int codePoint)`와 같이 int를 받는 메서드는 모든 유니코드 문자를 지원할 수 있음.
18. `Character.isLetter(0x2F81A)`는 `true` 반환.

문서 읽다 보면, code point와 code unit이 좀 헷갈리는 데, [여기](https://stackoverflow.com/questions/27331819/whats-the-difference-between-a-character-a-code-point-a-glyph-and-a-grapheme) 그 차이가 잘 설명되어 있음.

> For example, the snowman glyph (☃) is a single code point but 3 UTF-8 code units, and 1 UTF-16 code unit.

그리고 `U+`나 `\u` 문자들은 뒤이은 숫자들이 16진수임을 나타냄. 