...
### [How RxJava Works](</book/reactive-programming-with-rxjava/README.md#how-rxjava-works>)

동시성과 병렬성의 구분, 사람이 쓰기 좋게 API를 설계하려는 노력, Lazy 특성으로 인해 조합이 가능해지는 것, 쌍대성의 의미, API로 카디널리티를 구분하려는 의도 등이 인상 깊었음.

- [Concurrency and Parallelism](/book/reactive-programming-with-rxjava/README.md#concurrency-and-parallelism)
- [Lazy versus Eager](/book/reactive-programming-with-rxjava/README.md#lazy-versus-eager)
- [Duality](</book/reactive-programming-with-rxjava/README.md#duality>)
- [Cardinality](</book/reactive-programming-with-rxjava/README.md#cardinality>)

# 04/29

- [그래프 탐색](/book/algorithms-explained-animated/README.md#%EA%B7%B8%EB%9E%98%ED%94%84-%ED%83%90%EC%83%89)
- [Mechanical Sympathy: Blocking versus Nonblocking I/O](/book/reactive-programming-with-rxjava/README.md#mechanical-sympathy-blocking-versus-nonblocking-io)
...
- [Unreliable Clocks > Process Pauses](/book/ddia/Distributed-Data.md#process-pauses)
...
[The truth Is Defined by the Majority](/book/ddia/Distributed-Data.md#the-truth-is-defined-by-the-majority)
...
- [Learn how to create Flux instances](/document/reactive-programming-with-reactor-3.md#learn-how-to-create-flux-instances)
- [Learn how to create Mono instances](/document/reactive-programming-with-reactor-3.md#learn-how-to-create-mono-instances)
...
- [StepVerifier and how to use it](/document/reactive-programming-with-reactor-3.md#stepverifier-and-how-to-use-it)
- [Learn to transform our asynchronous data](/document/reactive-programming-with-reactor-3.md#learn-to-transform-our-asynchronous-data)

# 05/08

- Lite Rx API Hands On, [Merge](/document/reactive-programming-with-reactor-3.md#merge)

# 05/09

backpressure(volume control)에 대한 이야기. 수요를 제어하기 위한, `Subscriber`-> `Publisher` 피드백 메커니즘.

- Lite Rx API Hands On, [Request](/document/reactive-programming-with-reactor-3.md#request)

# 05/10

Reactor 3에서 에러가 발생했을 때, 폴백 처리를 하거나, Checked Exception이 발생한 경우에 대한 처리를 소개하고 있음.

- Lite Rx API HAnds On, [Error](/document/reactive-programming-with-reactor-3.md#error)

[Reactor 3 Reference Guide](https://projectreactor.io/docs/core/release/reference/)는 역시나 잘 정리된 문서라고 생각됨.

# 05/11

[Byzantine Faults](/book/ddia/Distributed-Data.md#byzantine-faults). 분산 시스템 환경에서, 노드가 신뢰할 수 없을 뿐만 아니라(unreliable), "거짓말"을 할 수도 있는 경우에 대한 이야기. 이런 상황에서 어떻게 합의를 이끌어 낼 것이냐에 대한 이야기. 일반적인 웹 애플리케이션 개발에는 문제가 되지 않지만, Peer-to-Peer 환경에서는 문제가 될 수도. Byzantine Fault Tolerant에도 여전히 전통적인 인증, 인가, 인크립션, 방화벽 등의 Weak forms of lying 극복은 중요함을 강조.

# 05/15

## Lite Rx API Hands Om

- [Adapt](/document/reactive-programming-with-reactor-3.md#adapt) 정리. 아래의 변환에 대해 다룸.
    - `Flux` <-> `Flowable`
    - `Flux` <-> `Observable`
    - `Mono` <-> `Single`
...
- [Other Operations](/document/reactive-programming-with-reactor-3.md#other-operations) 정리
...
- [Reactive to Blocking](/document/reactive-programming-with-reactor-3.md#reactive-to-blocking) 정리.
...
- 이제 마지막 부분!
- [Blocking to Reactive](/document/reactive-programming-with-reactor-3.md#blocking-to-reactive)
...
[System Model and Reality](/book/ddia/Distributed-Data.md#system-model-and-reality)
...
[Correctness of an algorithm](/book/ddia/Distributed-Data.md#correctness-of-an-algorithm)

알고리즘의 정확성을 기술하기 위해 속성을 이용. 예를 들어, 펜싱 토큰(fencing token) 알고리즘은 Uniqueness, Monotonic sequence, Availability 속성을 항상 만족해야 함.

[Safety and liveness](/book/ddia/Distributed-Data.md#safety-and-liveness)
...
[Mapping system models to the real world](/book/ddia/Distributed-Data.md#mapping-system-models-to-the-real-world)
...
- [Consistency and Consensus](/book/ddia/Distributed-Data.md#consistency-and-consensus). DDIA 책에서 마지막으로 살펴볼 내용.
- 패킷의 손실/꼬임/중복/지연, 시계의 부정확성, 노드의 멈춤/충돌을 어떻게 극복할 것인가.
- 가장 강한 일관성 모델인 linearizability, 분산 시스템에서의 순서 보장, 분산 트랜잭션의 원자적 커밋을 다룬다고 함. 시작!

# 05/26

## [Linearizability](/book/ddia/Distributed-Data.md#linearizability)
...
## [Relying on Linearizability](/book/ddia/Distributed-Data.md#relying-on-linearizability)
...
[/book/ddia/Distributed-Data.md#implementing-linearizable-systems](/book/ddia/Distributed-Data.md#implementing-linearizable-systems)
...
- [The Cost of Linearizability](/book/ddia/Distributed-Data.md#the-cost-of-linearizability)

# 06/18

## [Ordering and Causality](/book/ddia/Distributed-Data.md#ordering-and-causality)

- [The causal order is not a total order](/book/ddia/Distributed-Data.md#the-causal-order-is-not-a-total-order)
- [Linearizability is stronger than causal consistency](/book/ddia/Distributed-Data.md#linearizability-is-stronger-than-causal-consistency)
- [Capturing causal dependencies](/book/ddia/Distributed-Data.md#capturing-causal-depdencies)

# 06/19

## Spring Metrics, Prometheus

[여기 문서](https://docs.spring.io/spring-metrics/docs/current/public/prometheus)를 [읽으면서 간단 정리](/document/spring-metrics-prometheus.md)
...
- [Reactive to Blocking](/document/reactive-programming-with-reactor-3.md#reactive-to-blocking)
- [Blocking to Reactive](/document/reactive-programming-with-reactor-3.md#blocking-to-reactive)
