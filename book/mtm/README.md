...
- 보상 트랜잭션의 복잡성을 줄이기 위해 프로세스 단계를 바꾸기도.
- 원래 주문 단계가 아래와 같았다고 가정.
  - 주문서 작성
  - 재고 확인과 예약
  - 결제
  - 포인트 적립
  - 포장과 주문서 전달
  - 주문 완료
- 포장과 주문서 전달 실패 시 롤백 복잡성을 줄이고자,
- 포인트 적립을 마지막 단계로 이동하는 걸 고려할 수도 있다는 이야기.
- 보상 트랜잭션이 어려울 때 고민해 볼 만.

#### Mixing fail-backward and fail-forward situations

- 실패 복구 모드를 복합적으로 사용하는 게 좋음.
- 어떤 건 롤백이 필요하고, 어떤 건 실패에도 계속 진행하는 게 나을 수도.
- 예컨대, 고객의 결제가 완료됐고 남은 단계가 패키지 발송이라면,
- 이 때문에 전체 주문을 취소하기 보다는,
- 패키지 발송 재시도를 해보고,
- 그래도 안 되면 사람이 개입해서 해결해 보려는 시도를 할 수도.

### Implementing Sagas

- 이제 구현 얘기.
- 역시나 나오는 오케스트레이션 사가와 코레오그래피 사가.
- 오케스트레이션은 commandd-and-control, 코레오그래피는 trust-but-verify 아키텍처.

#### Orchestrated sagas

- 오케스트레이션은 중앙 집중형이며 오케스트레이터가 있고 현재 상황을 파악하기가 좀 더 쉬움.
- 하지만 커플링 발생. 코디네이터가 모든 관련 서비스를 알고 있음.
- 또 한 가지는 오케스트레이터가 점점 비대해지고 각 서비스들은 빈약해 질 수 있음.
- 중앙 집중화를 피하는 한 가지 방법은 다양한 서비스들이 각 상황에 맞는 오케스트레이터가 되는 것.
- 주문 처리는 주문 프로세서 서비스, 반품과 환불을 처리하는 반품 서비스, 창고에 도착하는 재고를 받고 선반에 적재하는 상품 수신 서비스 등.
- 그리고 재고 서비스 등은 이런 여러 오케스트레이터들에 의해 재사용 될 수 있음.

#### Choreographed sagas

- 한편 코레오그래피 사가는 여러 협력 서비스들에 연산 책임을 분산.
- 서비스 간 협력에 이벤트들을 매우 많이 활용.
- 이벤트는 브로드캐스팅 되고, 관심 있는 서비스들이 반응하는 형태로 동작.
- 보통은 메시지 브로커를 사용하니, 서비스를 알지 않아도 되고 시간적 결합도도 낮아짐.
- 다만 이 방식은 복잡함.
- 각 서비스들을 격리해서 이해한 뒤 전체 그림을 머리로 그려봐야 함.
- 비즈니스 절차의 명시성 부족 외에도, 상태 추적도 어려움.
- 물론, 이건 상태 변경에 대한 이벤트를 발행하고 수집해서 전용 뷰를 만들어도 됨.
- 이럴 때 correlation ID 등을 사용.

#### Mixing styles

- 둘을 혼합해서 사용할 수도.
- 예컨대, 재고 서비스 범위에서, 패키지를 관리하고 발송할 때는, 오케스트레이션 플로우를 사용. 그리고 이전 절차에서는 코레오그래피 사용.
- 하지만 복잡성이 커지므로, 명확한 정의와 공유가 필요.
...
- 독립적 배포를 위해서는 소비자들을 깨뜨리지 않는 변경이 필요.
- 우리가 다른 서비스에게 제공하는 기능을 계약이라고 볼 수 있음.
- 계약이 명시적이 아니더라도 그것은 존재.
- 변경 시 이 계약을 깨뜨리지 않게 신경써야 함.

### How Can This Problem Show Itself?

- 특정 서비스의 변경으로 다른 서비스들이 깨지거나 장애라면 바로 이 문제.
- 다행히 이 문제들은 보통 배포 후 즉각 드러나기 시작.
- 따라서 빠른 롤백 메커니즘이 도움이 됨.
- 또 다른 징후는 여러 서비스가 동시에 배포 되어야 하는 일이 잦은 것.
- lock-step release.

### When Might This Problem Occur?

- 하나의 서비스로 관리될 때는, 한 쪽의 변경이 미치는 영향이 눈에 잘 드러남.
- 그러나 영향이 서로 다른 팀에 일어날 때는 파악이 좀 더 어려움.
- 조직이 성숙해질수록, 깨드림을 피하는 변경을 만들 수 있고, 혹여나 문제가 생기더라도 빠르게 대응할 메커니즘들이 생길 것.

### Potential Solutions

아래는 계약 깨짐을 관리하는 규칙들.

- 의도치 않게 깨뜨리는 변경은 하지 않기.
- 깨뜨림 변경이 필요하다면, 혹시 피할 수 있는 방법을 찾기.
- 그래도 깨뜨림 변경이 필요하다면, 소비자에게 마이그레이션 할 시간을 주기.

#### Eliminate accidental breaking changes

- 명시적인 스키마를 만들어, 변경 시 계약 깨뜨림은 없는지 검토할 환경 만나들기.
- 깨뜨림에는 물리적인 것은 물론 논리적인 것도 함께 고려하기.
- 스키마 없는 JSON을 선호하던 사람도, 서비스들을 깨뜨려 보고 나면 마음이 바뀌기도 한다고.

#### Think twice before making a breaking change

- 가능하다면 확장을 고려.
- 메서드, 리소스, 토픽 등 기존 것을 제거하는 게 아니라 추가하는 것.
- 새로운 것과 함께 과거 것을 지원하는 방법 찾기.

#### Give consumers time to change

- 2개 버전의 서비스를 함께 운용하는 것도 방법.
- 인프라도 더 필요할 수도 있고, 코드 브랜칭이 필요할 수도 있음.
- 데이터 호환성도 함께 고려해야 하기도 하고, 두 버전 모두에 대해 버그 픽스를 해야할 수도.
- 저자가 더 선호하는 방법은, 한 서비스에서 2개 버전을 모두 지원하는 것.
- 만약 연관된 서비스들이 모두 한 팀에서 관리하는 거라면 lock-step release를 고민해 볼 수도.

## Reporting

- 모놀리스에서는, 단일 스키마에서, 주로 레플리카로, 한 번에 여러 데이터를, 조인 연산을 통해, 리포트를 만들곤 함.
- 하지만 MSA에서는 그럴 수 없음. 데이터가 여러 논리적으로 격리된 스키마에 흩어져 있기에.

### When Might This Problem Occur?

- 스키마 분리가 시작되면 바로 마주하는 문제.
- 미리 발견하면 좋겠지만 나중에서야 문제가 되는 경우들이 많음.
- 보통의 소프트웨어 개발과 시스템 유지보수 영역 밖에 있기에 놓치기 쉽상.
- 데이터 웨어하우스나 데이터 레이크 같은, 리포팅 목적의 전용 데이터 소스가 있다면, 이 문제는 피해갈 수도.

### Potential Solutions

- 일단, 리포팅 데이터를 저장할 데이터베이스를 서비스를 위한 것으로부터 분리.
- 이는 서비스 변경에 따른 데이터 계약 깨짐으로부터의 디커플링.
- 그리고 리포팅 요구사항에 맞는 변경을 좀 더 자유롭게 꾀할 수 있기도.
- 이제, 필요한 건 서비스에서 새로운 스키마로 데이터를 푸시하기.
- 앞에서 살펴본 CDC가 방법이 될 수도 있고, 제한적이긴 하지만 뷰가 도움이 될 수도.
- 리포팅 스키마로 데이터를 동기화하는 코드를 만들 수도.
- 또는 중간 컴포넌트를 두고, 업스트림 서비스로부터 이벤트를 받아, 리포팅 데이터베이스로 데이터 전달하는 것도 방법.

## Monitoring and Troubleshooting

- 모놀리틱에선 모니터링이 꽤 간단.
- 장비 수도 적고, 실패도 이분적(죽었냐 살았냐).
- 100개가 넘는 서비스를 갖고 있을 땐 다름.
- 한 프로세스에서 새벽에 CPU가 100%라면?
- 나머지 99개는 정상이라면?

### When Might These Problems Occur?

- 언제 이 문제를 가지는지 말하는 건 다소 모호.
- "프로덕션에서 뭔가 잘못됐을 때" 정도.
- 하지만 테스트 과정에서 언제 어디서 잘못됐는지 파악해야 할 수도.
- 서비스가 20개 정도 될 때까지 아무 문제 없기도 하고.

### How Can These Problems Occur?

- 설명하거나 이해하기 어려운 문제가 프로덕션에서 발생.
- 시스템이 명백히 안정적인데도 알림이 울림.
- 시스템은 지금 괜찮냐는 질문에 답하는 게 더 어려워짐.

#### Log aggregation

- 로그 애그리거트 시스템은 각 로그를 수집해서 중앙화.
- 한 번에 보고 검색할 수 있고 필요하면 알림도 보냄.
- ELK stack, Humio(저자의 선호) 등.
- 로그 애그리거트는 구현이 쉬움.
- 그리고 초기부터 해야만 하는 작업.

#### Tracing

- 여러 호출 단계중 어느 서비스에서 실패한 건지, 어느 구간에서 지연이 일어났는지 파악해야 함.
- 시작 단계로, 매 호출에 correlation ID를 (헤더나 메시지 페이로드를 통해) 주고 받기.
- 일반적으로 API 게이트웨이나 서비스 메시가 최초에 ID를 채번.
- 로그 남길 땐 이 ID를 일관된 형태로 남기기.

```
13:15[INFO][Notification][kk49249] Notify Customer called
```

- 좀 더 나아가서, 소요 시간을 함께 추적.
- 로그 수집은 주기적으로 일어날 수도 있으니 시간 판단을 위한 활용은 어려움.
- Jaeger 등의 도구가 도움이 됨.

#### Test in production

- 프로덕션에서도 품질에 관한 지속적 피드백 필요.
- synthetic transactions라 불리는 가짜 사용자 행위를 시스템에 주입.
- 예상된 행동을 정의한 뒤 이에 맞지 않는 경우 알림.
- 단, 프로덕션에 예상치 못한 문제가 일어나지 않도록 주의해야 함.
...
- 얼마나 많은 서비스를 로컬에 실행시켜야 하는지에 따라 다름.
- JVM 같은 리소스 집약적 환경인지, Go나 Node 또는 Python 같은 가벼운 환경인지에 따라 다르기도.
- 여러 서비스에 대한 집단 소유권일 경우 더 문제가 심각.

### Potential Solutions

- 서비스 stub out이 한 가지 방법.
- 혹은 다른 인프라에서 실행중인 것을 가리키게 하기.
- 후자는 연결이 많이 필요하고(출장지 등 연결이 어려운 곳에선 사용 불가),
- 원격 배포가 필요할 수도 있어 피드백 사이클 느려지며,
- 환경 구축을 위한 비용도 추가로 발생.
- Telepresence 같은 도구가 도움이 됨.
- 서비스가 늘어날수록 개발자 경험이 어떻게 변하는지 관찰하는 게 필요.
- 여전히 생산적인지 아닌지 피드백 얻을 수 있어야 함.

## Running Too Many Things

- 서비스와 인스턴스가 많아질수록,
- 배포하고 설정하고 관리하는 프로세스도 역시 많아짐.
- desired state management는 특히 중요.
- 이는 서비스 인스턴스의 갯수와 위치를 지정하고 유지하는 것.
- 이전에는 수동으로 했을지 몰라도, 서비스 수가 많아졌을 땐 함께 확장하기 어려운 방식.

### How Might This Problem Show Itself?

- 배포에 더 많은 사람들이 더 많은 시간을 쏟게 됨.
- 그리고 프로세스가 수동 활동에 의존할수록 실수는 더 많아짐.

### When Might This Problem Occur?

- 이 모든 건 확장에 관한 것.
- 마이크로서비스가 많아질수록, 수동 프로세스 혹은 정통적 자동 설정 관리 도구는 맞지 않음.
- Chef, Puppet 등이 그 예.

### Potential Solutions

- 좀 더 높은 수준의 자동화 도구 필요.
- 셀프 서비스 프로비전 배포를 돕고, 자동화된 desired state management.
- k8s가 도움이 되나 저자는 더 추상화된 개발자 친화적 도구들이 나올 거라고.
- 퍼블릭 클라우드 얘기도 꽤 하고 있음.

## End-to-End Testing

- 균형을 잡아야 함.
- 테스트는 많아질수록, 그리고 범위가 넓어질수록, 안정감을 느낄 수 있음.
- 그러나 범위가 넓고 실행 시간이 길어질수록, 실패 시 무엇이 문제인지 찾기 어려움.
- 종단 테스트는 작성과 테스트가 어렵지만, 사용자 관점에서의 품질을 검사한다는 점에서 가치가 있음.
- 하지만 마이크로서비스 아키텍처에서 종단 테스트의 범위는 매우 큼.
- 여러 서비스에 걸쳐 테스트가 일어나기에,
- 관련된 모든 서비스가 테스트를 위한 설정과 배포 등의 준비가 되어 있어야 하고,
- 배포 실패로 인한 네트워크 타임아웃 등의 환경 이슈로 의한 가짜 음성도 대응해야 함.
- 따라서 더 많은 시간과 자원이 소모됨.

### How Can This Problem Show Itself?

- 종단 테스트가 커지는 신호 중 하나는, 실행 시간 오래 걸리는 것.
- 여러 팀들이 기존 시나리오를 이해하지 못하고 필요 시 계속 추가하면서 나타나는 문제.

### When Might This Problem Occur?

- 각자 독립된 환경에서 테스트를 실행하는 게 아니라,
- 플로우가 여러 팀에 걸치게 되어 많은 팀들이 엮이게 될 때 문제가 됨.

### Potential Solutions

- Building Microservices에서 한 챕터를 할애해 다뤘었음.
- 여기서는 간략히만 소개.

#### Limit scope of functional automated tests

- 팀 경계를 가르는 큰 범위의 테스트는 피하기.
- 테스트를 팀 내의 소유로 유지하면 무슨 시나리오인지 이해하기도 쉬움.
- 따라서 디버깅하고 테스트하기도 쉬움.
- 실행과 통과에 대한 책임도 명확.

#### Use consumer-driven contracts

- 서비스를 가로지르는 테스트 케이스를 대체하고자 소비자 주도 계약(CDC)을 사용.
- 소비자의 관점을 이해하고, 소비자의 계약을 실수로 깨뜨리지 않게 예방.
- Pact 같은 도구가 도움이 됨.
- 이를 잘 사용한 팀도 있지만, 다른 곳에선 어려움도 많았음.
- 아직 구체적인 허들을 저자도 잘 모르는 상태라고.

#### Use automated release remediation and progressive delivery

- 시스템 복잡도가 커지면 모든 걸 미리 잡아내긴 어려움.
- 프로덕션 이슈의 영향도를 줄이는 것도 방법.
- 카나리 등 점진적 배포가 그 도구 중 하나.
- 95 백분위수나 에러율 등을 정해 자동 롤백도 가능.
- 이미 많은 곳에서 사용.

#### Continually refine your quality feedback cycles

- 새로운 테스트가 시스템을 커버하는지 식별할 수 있어야 하고,
- 중복된 커버리지가 있다면 제거할 수도 있어야 함.
- 빠른 피드백과 안정성 사이의 균형.

## Global Versus Local Optimization

- 지역적 의사결정과 전역 관심사 사이의 균형 고민을 하기 시작.
- 3개 서비스가 각자 필요에 맞게 Oracle, MongoDB, PostgreSQL을 사용한다고 가정.
- 하지만, 조직 관점에서, 3개 데이터베이스에 대한 역량 강화나 라이센스 비용이 합당한지 고민해야 함.
- 하나의 데이터베이스를 적용하면, 모두에게 최적은 아니지만, 대부분에게 충분히 좋은 경우라면?
- 지역적으로 일어나는 것들을 볼 수 있어야 이런 고민과 결정을 할 수 있음.

### How Can This Problem Show Itself?

- 같은 문제에 대해 각 팀이 서로 다른 방식으로 해결.
- 하지만 인지하지 못함.
- 그리고 점점 비효율적이 되어감.
- 예컨대, 서비스 배포 방식이 서로 다르면, 배워야 할 게 너무 많아지고, 비용을 정당화하기 어려워질 수도.
- 교차 팀 기술 그룹이나, 실무자 커뮤니티 등이 있으면, 좀 더 빨리 발견할 수 있음.

### When Might This Problem Occur?

- 처음부터 문제가 되지는 않음.
- 처음엔 명확한 공유된 이해로 시작.
- 그러나 점점 지역적인 최적화가 이어남.
- 조직이 커지고, 특히 개발자 유입이 단기간에 많이 이뤄진 경우, 이런 사일로 현상을 자주 봄.

### Potential Solutions

- 2장에서 가역적 비가역적 결정을 다룸.
- 사람들에게 그들의 결정이 비가역적 측에 속하진 않는지를 고려하게 도와야 함.
- 그리고 비가역적이라면 팀 경계를 넘어선 사람들과 함께 의사결정 할 수 있어야 함.
- 팀들은 더 큰 그림에 대한 합의가 있어야 하고, 동료들과 필요할 때 얘기할 수 있는 네트워크가 있어야 함.
- 이를 위한 간단한 방법으로는, 팀 별로 기술 리더를 두고, 교차 관심사 그룹 일원으로 두는 것.
- CTO나 최고 아키텍트 또는 회사 기술 비전에 전반적 책임 있는 사람이 주관.
- 이 팀은 팀들에게 당면하고 있는 지역적 이슈 또는 횡단 관심사 이슈를 다같이 얘기할 수 있게 함.
- 팀간 커뮤니케이션이 없다면, 각자 다른 방식의 최적화를 하는지 알 길이 없음.
- 또한 전역 레벨에서 해결해야 할 문제인지 판단하기도 어려움.
- 조직 성향에 따라 좀 더 임시 방편의 비공식 프로세스를 활용할 수도.
- Monzo라는 곳에선 "제안"이라 부르는 자유 양식 문서를 제출할 수 있게 했다고 함.
- 이게 제출되면 공유된 공간에 게시가 되고 전사에 슬랙을 통해 새로운 제안이 등록됐다고 알림.
- 관심 있는 곳에서 토론을 하고 계속 수정하고 정리.
- 조직은 지역적 그리고 전역적 의사 결정 사이의 적절한 균형을 찾아야 함.
- 각 팀에 더 많은 책임을 부여하면, 자율적이긴 하지만 조직 전체의 일관성 문제가 생김.
- 중앙에서 더 많은 것을 드라이브할수록, 더 많은 동의가 필요하고 느려짐.

## Robustness and Resiliency

- 모놀리스에 비해 다양한 장애들 존재.
- 네트워크 패킷 손실, 타임아웃, 머신 장애 등도 자주 발생.

### How Can This Problem Show Itself?

- 불행히도 프로덕션에서 문제가 일어남.
- 개발이나 테스트 과정에서는 잘 안 일어남.

### When Might This Problem Occur?

- 언제 일어날지는 알기 어려움.
- 서비스가 많아지고 서비스 간 호출이 많아지면 더 가능성이 커짐.
...
- 호출이 실패하는 방법을 아는지?
- 실패 시 뭘 해야 할지 아는지?
- 비동기 커뮤니케이션(순간적 결합을 피하는)은 서비스 간 격리 방법 중 하나.
- 합리적인 타임아웃은 다운스트림 서비스의 지연 시 리소스 경합을 피하는 방법.
- 혹은 써킷 브레이커나 백프레셔도 있음.
- 여러 서비스 복제를 실행시키기도 필요.
- 서비스가 충돌나면 재시작을 하는 등의 desired state management 사용도 고려.
- 좀 더 나아가서 회복성은 단순한 패턴 구현을 넘어 조직의 일하는 방식에도 관련.
- 예컨대 프로덕션 이슈들을 기록하고 배운 것을 유지하려는 노력 등이 필요.

## Orphaned Services

- 서비스가 많아지면 누가 무엇을 소유하고 있는지 파악하기 어려움.
- 회사 내 어느 누구도 소유하지 않는 서비스도 있으며 이를 고아 서비스라 부름.

### How Can This Problem Show Itself?

- 무엇을 하는지 아무도 모르는 서비스가 있기도 함.
- 무엇을 하는지 모르니 이걸 꺼도 되는지도 모름.

### When Might This Problem Occur?

- MSA를 오래 하다 보면 이 문제를 만날 수 있음.

### Potential Solutions

- 소스 코드 리포지토리를 크롤링 하면서 메타데이터 파일을 찾아 서비스 목록을 구축하는 도구를 만든 적도 있음.
- 여기에 컨설이나 etcd와 결합해 보다 풍부한 지도를 구축하기도.
- 그 외에도 여러 얘기를 하는데 결국 서비스들을 한 눈에 보는 것.
- 그리고 고아 서비스를 발견하면 소유권을 부여(강한 소유권을 추구할 경우).
