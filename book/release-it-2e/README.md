...
4. 구글이나 마이크로소프트가 아닌 이상에야, 테스트 환경을 유사하게 구축하기는 어려움.
5. 테스트 보다는 설계로 해결해야 함. 점대점 통신을 아래의 것들로 바꾸는 것을 고려해 볼 수 있음.
   - UDP 브로드캐스팅: 대역폭 사용이 비효율적.
   - TCP 또는 UDP 멀티캐스팅: 좀 더 효율적. 브로드캐스팅에 비해 오직 관심 있는 서버에 대해서만 메시지 수신을 허용하기 때문.
   - 구독/수신 메시징: 좀 더 나은 선택. 서버가 잠시 고장나더라도, 정상화 된 후에 다시 메시지를 주워갈 수 있기 때문. 하지만, 적지 않은 인프라 비용이 수반되며 "Do the simplest thing that will work"을 생각해 볼 필요가 있음.
   - 메시지 큐
...
1. 수용력에 불균형이 있을 수 있음.
   - 예를 들어, 프론트 시스템이 존재하고, 여기서 모든 요청을 먼저 받은 다음, 뒤에 있는 스케쥴링 시스템과 상품 검색 시스템으로 요청을 분배해준다고 해보자.
   - 평소에는 대부분의 요청이 상품 검색으로 가지만, 마케팅이나 프로모션 등에 의해 스케쥴링 시스템으로의 요청이 평소 대비 10배 이상 뛸 수도 있음.
   - 그러나, 스케쥴링 시스템은 이 부하를 다 처리할 준비가 되어 있지 않을 수도.
   - 그렇다고, 예상 가능한 최대 부하를 견디도록 인프라를 설계할 수도 없는 일. 대부분이 유휴 시간일테니.
2. 예상 가능한 부하를 감당할 수 있을 만큼 시스템을 크게<sup>large enough</sup> 설계하는 대신, 호출부나 제공자 모두 요청의 쓰나미에 맞춰 탄력 있게<sup>resilient</sup> 만들 것.
   - 호출자의 입장에서는 Circuit Breaker가 다운스트림 서비스에 대한 압력을 경감시켜 준다는 점에서 도움이 될 것.
   - 서비스 제공자의 입장에서는 Handshaking과 Backpressure를 통해 호출자에게 요청량 조절<sup>throtle</sup>을 안내할 수도 있음.
   - 또한, 우선 순위가 높은 요청이나 중요한 서비스에 대한 수용력을 보존하기 위해 Bulkhead를 고려할 수도.
...
4. 한편, 예상할 수 없는 트래픽의 변동에 대해서도 준비해야 함.
   - 첫 번째 이야기는 이해 안 됨. 영어로 그대로 가져옴. "First, use capacity modeling to make sure you're at least in the ballpark. Three thousand threads calling into seventy-five threads is not in the ballpark."
   - 두 번째로, 부하 상황을 테스트 할 것. 시스템이 충분히 탄력적이라면 조금 느려지다가(혹은 빠른 실패를 하거나), 부하가 사라지면 다시 원래대로 복구돼야 함.
   - 세 번째로, 오토 스케일링을 사용 권장.
...
1. **Ask for help before causing havoc.**
   - 인프라 관리 도구는 단시간에 지대한 영향을 미칠 수 있음.
   - 전체 시스템이 한 번에 붕괴되지 않도록,
   - 제한자<sup>limiters</sup>와 안전보조장치<sup>safeguards</sup>를 마련.
2. **Beware of lag time and momentum.**
   - 자동화에 의한 액션이 시작되는 데는 시간이 걸릴 수 있음.
   - 보통 이 시간은 모니터링 주기보다 길기 마련.
   - 따라서, 시스템이 액션을 취하는 데 걸리는 지연을 감안하게 할 것.
3. **Beware of illusions and superstitions.**
   - 제어 시스템은 기대하는<sup>exepected</sup> 상태를 계산함.
   - 또한, 현재<sup>current</sup> 상태에 대한 "믿음"(현재 상태에 대한 파악은 정확하지 않을 수 있음을 표현하는 것)을 계산함.
   - 두 결과 모두 잘못될 수 있음을 인지해야 함.
...
2. 일반적인 데이터베이스 질의 코드는 다음과 같음.
   ...
   - 결과 셋에 대해 루프를 돔.
   - 각 로우를 처리.
3. 로우를 처리한다는 것의 의미는 새로운 데이터 객체를 컬렉션에 추가한다는 것.
...
지금은 모든 시스템이 분산 시스템. 네트워크는 종종 실패하고, 애플리케이션은 이런 특성을 대비할 수 있어야 함. 와이어, 스위치, 라우터, 그리고 우리가 다루는 컴퓨터 또한 부서지기 쉬움. 이 때, 언제 돌아올지 모르는 응답을 마냥 기다릴 수는 없음. 희망을 버리고 포기할 수 있어야 함. 잘 적용된 타임아웃은 장애 격리를 가져다 줌. 다른 서비스나 장비의 실패가 꼭 우리의 문제가 될 필요가 없음.

광범위하게 적용되는 타임아웃을 다루는 방법들은 다양함.

1. 오래 걸리는 연산들을 기본 요소 집합으로 만들어서 많은 곳에 재사용.
   - 예를 들어, 데이터를 DB로부터 가져오는 일련의 작업.
   - 리소스 풀로부터 DB 연결을 체크아웃하고, 쿼리를 실행하고, 결과를 객체로 변환하고, DB 연결을 다시 풀로 반환.
   - 이런 일련의 과정을 모든 곳에 반복해서 작성하는 대신, [Query Object(P of EAA catalog)](https://martinfowler.com/eaaCatalog/queryObject.html)를 적용.
2. 제너릭 게이트웨이를 사용해서 템플릿을 제공.
   - 호출부는 핵심 로직을 이 템플릿을 통해 실행.
   - 이런 식으로 하면 Circuit Breaker 패턴 적용도 쉬워짐.
3. 플랫폼을 최대한 활용할 수도.
   - Amazon API Gateway 같은 서비스는 우리를 대신해서 여러 지저분한 일들을 대신 처리해 줌.
   - 콜백이나 리액티브 프로그래밍 같은 도구들은 타임아웃을 쉽게 지정할 수 있게 도와줌.

타임아웃과 더불어 재시도를 사용할 때는 느린 재시도도 함께 고려.

1. 보통 빠른 재시도를 시도하기도 하지만,
2. 장애는 얼마간 지속되기 마련이고,
3. 따라서, 타임아웃 이후의 빠른 재시도 역시 실패할 가능성이 매우 높음.
4. 반면, 느린 재시도를 위해 작업을 큐에 쌓는 것은 시스템을 더 강건하게 만들 수 있음.
5. 즉, 지연된 재시도를 함께 고려하자.

타임아웃은 Circuit Breakers와 잘 어울림.

1. 타임아웃을 도표화 할 수 있고,
2. 많이 발생한다면 "off" 상태로 만들 수도.

타임아웃과 빠른 실패는 모두 레이턴시 문제를 다루는 패턴.

1. 빠른 실패가 들어온 요청에 적용되고,
2. 타임아웃은 요청을 보내는 경우에 적용.

참고로, 타임아웃은 또한 무한한 결과 셋에 대한 대응이기도 함. 하지만, 그리 효과적인 것은 아님. 임시 방편 정도.
...
6. 지금은 써킷 브레이커가 사용되며, 집이 불타는 것을 막아줌.
...
1. 일반적인 "closed" 상태에서 써킷 브레이커는 평상시 처럼 연산을 수행.
2. 하지만, 일정 횟수 이상 실패가 일어나면 써킷 브레이커는 써킷을 "opens" 상태로 바꿈.
...
써킷 브레이커를 적용할 때는 시스템의 이해관계자가 함께 결정해야 함.
...
써킷 브레이커는 통합 지점을 보호하고, 장애 전파를 막아주며, 불균형 수용량을 조절해주고, 느린 응답에 대응할 수 있게 해 줌. 주로 타임아웃과 밀접하게 관련되며, 실행 실패와는 별개로 관리됨.
