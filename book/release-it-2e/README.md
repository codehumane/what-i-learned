...
큐의 부하는 클라이언트에게도 전파되며, 부하가 해소되기 전까지는 클라이언트들의 속도 또한 낮아질 것. 참고로, 일시적 상태에 의한 *back pressure*와 컨슈머가 망가져서 생긴 그것과는 구분하는 것이 중요. Rx 프레임웍이 이런 부분에 도움이 될 것.

back pressure에 의해 스레드는 분명 대기 상태가 될 수 있으며, 이는 다운타임으로 가는 지름길. 시스템의 경계에서 사용한다면 사용자에게는 불편한 경험이 될 것. 따라서, 시스템 경계 안에서 사용하는 것이 적절. 시스템 경계에서는 *shed load*를 사용하자.

태그를 생성하는 경우의 back pressure 사용 사례는 다음과 같음.
