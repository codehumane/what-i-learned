...
개발자 친화적 테스트의 속성에 대해 계속 얘기했었음.

- 신뢰할 수 있는<sup>reliable</sup>
- 빠른<sup>fast</sup>
- 확장가능한<sup>scalable</sup>

좋은 단위 테스트는 이 속성들을 모두 만족시킴. 그러나 규모가 큰 테스트는 종종 이 제약들을 모두 무시.

- 더 많은 인프라를 사용하므로 신뢰성이 떨어짐.
- 준비와 실행에 오랜 시간이 걸림.
- 자원과 시간 제약, 다른 테스트와의 충돌(격리되지 않아서) 등의 이유로 스케일링이 어려움.

규모 있는 테스트는 이 외에도 2가지 어려움이 있음.

- 첫 번째로 소유권. 보통 여러 단위와 여러 소유자에게 걸쳐 있기에, 누가 테스트를 만들고 유지보수 할 것인지 모호.
- 두 번째로 표준화. 인프라나 프로세스에 대한 표준이 없기 마련. 제품마다 서로 다른 방식과 도구들 사용.
