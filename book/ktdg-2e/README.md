...
- 신뢰성은 시스템의 속성.
- 따라서, 카프카만 노력한다고 되는 게 아님.
- 애플리케이션 개발자, 리눅스 운영자, 카프카 운영자, 네트워크와 스토리지 운영자 모두가 참여해야 함.
- 한편, 신뢰성 있는 데이터 전달에 있어 카프카는 매우 유연.
- 신뢰성을 극도로 필요로 하는 곳부터, 신뢰성보다는 속도와 단순성을 추구하는 곳에도 어울림.
- 이 때문에 막연하게 카프카를 쓰면 신뢰성 보장되겠거니 하는 생각은 위험.
- 이번 장에서는 카프카의 다양한 신뢰성 수준, 그리고 신뢰성과 관련 깊은 복제 메커니즘을 다룸.
- 또한, 신뢰성과 관련해 브로커와 토픽 설정은 어떻게 해야 하는지도 이야기.
- 클라이언트, 프로듀서, 컨슈머도 마찬가지.
- 마지막으로, 시스템 신뢰성을 검증하는 주제도 소개.

## 7.1 신뢰성 보장

- 신뢰성을 위해 카프카가 하는 것은 다음과 같음.
- 동일한 프로듀서가, 동일한 파티션에 메시지를 썼다면, 파티션 안 메시지들 순서는 보장.
- 클라이언트가 쓴 메시지는 모든 in-sync 레플리카 파티션에 쓰여져야 커밋된 것으로 간주.
- 커밋된 메시지는 작동 가능한 레플리카가 최소 1개 있는 한 유실되지 않음.
- 컨슈머는 커밋된 메시지만 읽을 수 있음.
- 하지만 신뢰성은 시스템 전체가 협력해야 달성 가능.
- 그리고 카프카 신뢰성은 개발자나 운영자가 조정이 가능.
- 신뢰성, 일관성, 가용성, 처리량, 지연, 저장 비용 등의 트레이드 오프 선택해야 함.

## 7.2 복제

- 복제가 핵심이라고 또 한 번 언급.
- 여기서는 카프카 신뢰성 보장의 핵심으로 복제 등장.
- 그리고 디스크가 아니라, 여러 레플리카에 메시지 쓰기를 통해 지속성을 달성.
- 페일 오버 시, in-sync 팔로워가 리더가 되는데, in-sync의 조건은 아래와 같음.
  - 주키퍼와 활성 세션이 있음. 예를 들어, 최근 6초 사이 주키퍼로 하트비트 전송함.
  - 최근 10초 사이 리더로부터 메시지를 읽어 옴.
  - 최근 10초 사이 리더로부터 읽은 메시지가 가장 최근 메시지여야 함(랙이 없었던 적이 한 번 이상 있어야).
- 동기화가 살짝 늦은 in-sync 레플리카는 프로듀서와 컨슈머를 느리게 만들 수 있음.
  - 메시지 커밋 전, 모든 in-sync 레플리카가 메시지를 받을 때까지 기다려야 하기 때문.
- 느린 레플리카와의 동기화가 풀리면 기다림은 없어지므로 더 이상 프로듀서와 컨슈머 성능엔 영향 없음.
- 다만, 복제 팩터가 줄어드니 데이터 유실 위험성은 높아짐.

## 7.3 브로커 설정

- 메시지 저장 신뢰성 관련한 브로커 설정은 3가지.
- 적용은 브로커 단위로 시스템 전체 토픽 제어도 가능하고,
- 토픽 단위로 적용해서 특정 토픽에 대한 작동만 제어할 수도 있음.
...
- 기본값은 3.
- 값 결정에는 여러 고려 사항이 있음.
- 가용성: 레플리카 많을수록 가용성 올라감.
- 지속성: 한 번 쓴 데이터의 유실 없이 유지 되느냐 이야기. 레플리카 많을수록 올라감.
- 처리량: 레플리카 늘면 복제 때문에 트래픽 올라간다는 이야기.
- 종단 지연: in-sync 레플리카에 복제 되어야 컨슈머가 읽을 수 있으므로, 복제가 느리면 컨슈머도 함께 느려짐.
- 비용: 더 많은 레플리카는 더 많은 저장소와 네트워크를 필요로 함. 복제 팩터를 2로 잡아 비용을 합리적으로 줄이려는 시도도 있다고 함.
- 레플리카의 랙/가용영역 위치 이야기도 함.
- 온프렘에서는 카프카를 서로 다른 랙에 위치시키고 broker.rack에 서로 다른 랙 이름을 잡아주길 권장.
- 클라우드 환경에서의 가용 영역도 마찬가지로 활용.

### 7.3.2 언클린 리더 선출

- `unclean.leader.election.enable`
- 리더 선출이 되려면 3가지 조건이 필요하다고 얘기했었음.
- 활성 세션(일정 주기로 하트 비트 전송)이 있고, 최근 10초 사이에 리더로부터 메시지 읽었고, 한 번이라도 랙 없이 최신 메시지를 읽었어야 함.
- 그러나 이 조건을 달성하지 못한 리더 후보들이 있을 때, 가용성을 위해 데이터 유실과 비일관성을 감수하고 리더 선출을 해야 할 수도.
- 평소에는 이 값을 기본 값인 false로 두고, 문제가 있을 때 의사결정 후 true로 바꿔 재시작.

### 7.3.3 최소 in-sync 레플리카

- `min.insync.replicas`
- 토픽에 3개의 레플리카가 있는데, 만약 1개만 남아 있는 상황이라면, 카프카가 모든 in-sync 레플리카에 쓰여지면 커밋으로 간주하긴 하지만, 여기서는 결국 1개만 데이터를 가지는 것.
- 이 문제를 막고자 `min.insync.replicas`를 2로 잡아줄 수 있음.
- 이 때 만약 1개 레플리카만 남아 있다면, 클라이언트가 메시지를 쓰고자 할 때 `NotEnoughReplicasException`을 받게 됨.
- 사실상 읽기 전용 상태가 되는 것.
- 데이터 유실을 막기 위한 선택.

### 7.3.4 레플리카를 in-sync 상태로 유지하기

- in-sync 레플리카를 판단할 때 얼마나 민감하게 할 건지에 대한 설정.
- `zookeeper.session.timeout.ms`: 브로커가 주키퍼로 하트 비트 전송을 보내야 하는 최대 기한.
- 클라우드 환경에서의 안정성을 위해 기본값이 6초에서 18초로 변경되었다고 함.
- `replica.lag.time.max.ms`: 이 기간 내에 데이터를 읽어 오지 못하거나, 읽어 왔더라도 최신 메시지를 따라 잡은 적이 없다면, out-sync로 간주.
- 이 기본값도 10초에서 30초로 조정 되어, 불필요한 변동을 피하고 회복탄력성을 높였다고 함.
- 이는 컨슈머가 메시지를 읽는데 최대 30초 걸릴 수 있음을 의미하기도. 주의.

### 7.3.5 디스크에 저장하기

- 카프카의 메시지 지속성은 레플리카 수에만 의존.
- 디스크에 메시지가 저장되지 않아도 응답함.
- 디스크 저장은, 세그먼트를 교체할 때(기본값 1GB), 그리고 재시작 직전에만 일어남.
- 그 외의 경우는 리눅스 페이지 캐시 기능에 의존. 즉, 페이지 캐시 공간이 다 찼을 경우 메시지를 플러시.
- 이는 서로 다른 랙 또는 가용 영역에 위치한 세 대의 장비가, 리더의 디스크에 메시지를 쓰는 것보다 더 안전하다는 판단에서 비롯.
- `flush.messages`, `flush.ms`를 설정해서 메시지를 더 자주 저장하도록 설정할 수는 있으나 fsync가 카프카 처리량에 미치는 영향을 알아둬야 함.

## 7.4 신뢰성 있는 시스템에서 프로듀서 사용하기

- `acks=all` 설정.
- 언클린 리더 선출 기능 끄기.
- 그리고 프로듀서가 받는 에러를 적절히 대응하기.
...
- LEADER_NOT_AVAILABLE 같이 재시도 가능한 에러에 대해서는 `delivery.timeout.ms`까지 계속 재시도.
- 참고로, 재시도 수 기본값은 MAX_INT임.
- 주의할 점은, 이 재시도로 인해 메시지가 중복될 수 있다는 점.
- 최소 한 번은 보장하지만 정확히 한 번은 보장할 수 없음.
- `enable.idempotence=true`를 설정해서 프로듀서가 추가 정보를 담아 보내면 중복 메시지를 브로커가 건너띄게 할 수 있음.

### 7.4.3 추가적인 에러 처리

- 메시지 크기나 인가 관련 에러 등 재시도 불가능한 에러가 있음.
- 혹은 메시지가 브로커로 전송되기 전에 직렬화 과정 등에서 에러가 날 수도.
- 또한 재전송 시도가 모두 소진됐거나 프로듀서의 가용 메모리 부족으로 인한 에러도 있음.
- 물론, 네트워크 타임아웃도 있음.
- 이런 경우들은 어떻게 처리할지 결정해야 함.
- 그냥 폐기할 수도, 에러 로깅할 수도, 이어지는 처리를 중단할 수도, 백프레셔로 일시적 중단을 할 수도, 별도 저장소에 실패 메시지를 저장해야 할 수도, ...

## 7.5 신뢰성 있는 시스템에서 컨슈머 사용하기

- 계속 언급했듯 컨슈머는 커밋된 데이터만 읽을 수 있음.
- 즉, 모든 in-sync 레플리카에 쓰여진 다음부터 읽을 수 있음.
- 따라서, 컨슈머는 일관성이 보장되는 데이터만 읽는 것.
- 컨슈머가 할 남은 일은, 메시지를 어디까지 읽어서 처리했는지 잘 추적하는 것.
- 컨슈머가 죽고 새로운 컨슈머가 와도 이 마지막 오프셋만 있으면 됨.
- 물론, 중복 처리가 될 수 있음(처리는 됐지만 커밋이 안 되었을 수 있으니).
- 메시지 누락이 발생하는 경우는, 읽기는 했으나 처리가 완료 안 된 메시지의 오프셋을 커밋하는 경우.

#### 커밋된 메시지 vs. 커밋된 오프셋

- 커밋된 메시지: in-sync 레플리카에 쓰여져 컨슈머가 읽을 수 있는 메시지.
- 커밋된 오프셋: 컨슈머가 파티션의 어느 메시지까지 읽었는지에 대한 값.

### 7.5.1 신뢰성 있는 처리를 위해 중요한 컨슈머 설정

4가지 설정이 중요. 먼저 `group.id`

- 같은 그룹 아이디를 공유하는 컨슈머는, 같은 토픽이더라도 서로 다른 파티션 메시지를 수신.
- 하나의 컨슈머가 토픽의 모든 메시지를 읽어야 한다면 고유한 그룹 아이디가 필요.
- 별 건 아니지만 유의.

`auto.offset.reset`

- 커밋된 오프셋이 없거나 브로커엔 없는 오프셋으로 요청한 경우, 어디서부터 읽을지에 대한 설정.
- `earliest`, `latest` 중에 선택.

`enable.auto.commit`

- 컨슈머가 알아서 주기적으로 오프셋 커밋을 하게 할 건가, 아니면 코드에서 직접 커밋할 건가.
- 자동을 사용하면, 아직 처리 중인 오프셋을 실수로 커밋하지 않게 함.
- 다만, 중복 처리가 일어날 수 있다는 게 단점.
- 만약, 메시지 처리를 백그라운드에서 하고자 별도 스레드에 레코드를 넘기는 경우엔 수동 오프셋 커밋이 필요할 수도.

`auto.commit.interval.ms`

- 오프셋 자동 커밋 시 주기.
- 더 자주 커밋할수록 오버헤드는 늘어나고 중복 처리 가능성은 줄어듦.

### 7.5.2 컨슈머에서 명시적으로 오프셋 커밋하기

- 수동으로 오프셋을 커밋하기로 했다면 고려해야 할 것들 이야기.
- 당연히, 메시지 처리를 먼저 하고, 오프셋 커밋은 나중에 해야 함.
- 커밋을 자주 하면 성능은 줄어드는 대신, 페일 오버 시 중복은 줄어듦.
- 이것도 당연한데, 읽은 메시지가 아니라 처리한 메시지 오프셋을 커밋해야 함.
- 컨슈머 리밸런스가 발생한다는 것을 고려해야 함. 파티션 해제 전 오프셋을 커밋하고, 애플리케이션에 관련 상태가 있다면 적절히 관리.
- 컨슈머는 재시도를 해야 할 수 있음. 예를 들어, #30 메시지 처리에 실패한 상태에서 #31 메시지 처리를 성공했다고, #31 오프셋을 커밋하면 안 됨.
- 잠시 컨슈머를 `pause()` 메서드로 멈추고 처리가 필요한 레코드를 버퍼에 저장한 뒤 다시 처리.
- 혹은 실패한 메시지를 별도 토픽에 쓴 뒤 처리를 재개.
- 마지막으로, 컨슈머가 상태를 유지해야 할 수도 있음.
- 다시 말해서, 만약 메시지에 들어 있는 값의 평균을 계산하고자 한다면, 프로세스 재시작 시 마지막 오프셋이 아니라 누적 값도 복구한 뒤 메시지를 읽어야 함.
- 재시작 전, 누적된 값을 애플리케이션이 오프셋 커밋 시 `results` 토픽에 쓰는 것도 방법.

## 7.6 시스템 신뢰성 검증하기

- 3가지 검증을 수행하기를 권장.
- 설정 검증, 애플리케이션 검증, 프로덕션 애플리케이션 검증.

### 7.6.1 설정 검증하기

- 설정 검증은 일단 쉬움.
- 그리고 우리 요구 조건이 충족되는지 확인 가능.
- 시스템 예상 동작도 추론해 볼 수 있음.
- `org.apache.kafka.tools`에 들어 있는 `VerifiableProducer`, `VerifiableConsumer` 활용.
- 책 설명이 좀 모호한 것 같아 [여기 코드](https://github.com/a0x8o/kafka/blob/master/tools/src/main/java/org/apache/kafka/tools/VerifiableProducer.java#L536) 함께 살펴 봄. 동작 이해가 명확히 됨.
- 이를 활용해 리더 선출, 컨트롤러 선출, 롤링 재시작, 언클린 리더 선출 테스트 등을 진행.
- 예컨대, 검증용 프로듀서 실행 후 파티션 리더를 잠시 정지. 이 때 잠시 중단만 있을뿐 메시지 유실 없이 작동은 재개 되어야 함. 프로듀서와 컨슈머 메시지 로그를 보며 이 동작을 확인.

### 7.6.2 애플리케이션 검증하기

- 브로커와 클라이언트 설정을 확인했다면, 이제 애플리케이션을 검증해야 함.
- 커스텀 에러 처리, 오프셋 커밋, 리밸런스 리스너와 같은 곳을 확인하는 것.
- 클라이언트 연결 끊어짐, 블라이언트와 브로커 사이의 지연, 디스크 공간 부족, 리더 선출, 브로커 롤링 재시작, 컨슈머 롤링 재시작, 프로듀서 롤링 재시작 등의 케이스 확인.

### 7.6.3 프로덕션 환경에서 신뢰성 모니터링 하기

- 프로덕션 시스템 모니터링도 당연히 빠뜨릴 수 없음.
- 프로듀서의 경우 신뢰성 관련 가장 중요한 2가지 지표는, 레코드별 에러율과 재시도율.
- 에러 로그 등을 통해 확인할 수 있음.
- 컨슈머의 경우는 컨슈머 랙<sup>lag</sup>이 중요.
- 링크드인이 개발한 Burrow 도구 등이 도움.
...
### 8.1.2 Limitations of the Idempotent Producer

- 멱등성 프로듀서는 오직 프로듀서의 내부 로직에 의해 재시도가 발생했을 때의 중복만 막아줌.
- 동일 메시지에 대해 `producer.send()`를 두 번 호출하면 중복이 만들어지고 멱등성 프로듀서는 이를 막지 않음.
- 왜냐하면 프로듀서 입장에서는 이 2개의 레코드가 동일한 것인지 확인할 수 없기 때문.
- 그래서 프로듀서 예외를 잡아 애플리케이션이 재시도를 하는 것보다 내장된 재시도 메커니즘 사용을 권장.
- 재시도 시의 중복을 막는데는 멱등성 프로듀서 방법이 제일 간단.
- 애플리케이션 인스턴스가 여러 개이고, 하나의 인스턴스에 여러 개의 프로듀서가 있는 건 흔한 일.
- 만약 2개의 프로듀서가 동일한 메시지를 보낸다면 멱등성 프로듀서는 중복 감지 불가.
- 파일 등의 원천으로부터 애플리케이션이 데이터를 얻어 카프카로 전송할 때 흔히 발생할 수 있음.

### 8.1.3 How Do I Use the Kafka Idempotent Producer?

- `enable.idempotence=true` 설정으로 멱등성 프로듀서 사용 가능.
- 만약 이미 `acks=all` 설정이 있다면, 성능상 차이는 없음.
- 멱등성 설정이 활성화되면 다음의 일들이 일어남.
- 프로듀서 ID 획득을 위해 프로듀서는 기동 시 추가 API를 하나 더 호출.
- 각 레코드 배치는, 프로듀서 ID와 함께, 배치의 첫 메시지에 대한 시퀀스 ID를 가짐. 이를 위해 각 레코드 배치에 대해 96비트를 더하는데 큰 부담은 아님.
- 브로커는 단일 프로듀서 인스턴스로부터의 시퀀스 번호를 검사해 중복을 방지.
- 각 파티션에 전달된 메시지 순서가 보장되며, `max.in.flight.requests.per.connection` 값이 1 이상이어도(단, 5 이하여야 함) 보장.

## 8.2 Transactions

- 트랜잭션은 정확성 보장을 위한 기능.
- 여기서의 정확성은 장애 상황에서도 정확히 한 번 처리를 말함.
- 주의할 점은 트랜잭션이 스트림 처리 애플리케이션을 위해 만들어 졌다는 것.
- 구체적으로는 "consume-process-produce" 패턴(스트림 처리의 기본 형태)에 맞춰짐.
- 각 입력 레코드를 처리하고 나면 그 결과를 출력 토픽에 전달해줘야 함.
- 트랜잭션이 해결할 수 없는 것도 다룰 것.

### 8.2.1 Transactions Use Cases

- 정확성이 중요한 스트림 처리 애플리케이션이라면 어디든 유용할 수 있음.
- 특히 aggregation 그리고/또는 join을 포함한 스트림 처리.
- 금융 애플리케이션의 정확한 집계 기능이 대표적 사례.
- 물론, 정확히-한번 기능은 설정이 쉬워서, 챗봇 등의 일반적인 경우에도 종종 사용.

### 8.2.2 What Problems Do Transactions Solve?

- 간단한 스트림 처리 애플리케이션을 가정해 보자.
- 원천 토픽으로부터 이벤트를 읽고, 처리한 다음, 다른 토픽에 결과를 쓰는 애플리케이션.
- 각 메시지는 정확히 한 번만 쓰여지길 기대함.
- 여기서 뭐가 잘못될 수 있을까?
- 2가지 시나리오 살펴보자.

#### Reprocessing caused by application crashes

- 메시지를 컨슘 해서 처리한 뒤, 애플리케이션은 2가지를 해야 함.
- 1) 결과를 출력 토픽에 발행한 뒤, 2) 컨슘 메시지 오프셋 커밋.
- 그런데 만약, 출력 토필에 발행은 성공했지만 오프셋을 커밋하기 전에 애플리케이션이 고장났다면?
- 하트비트 전송이 안 되니, 리밸런싱이 시작되고, 파티션은 새로운 컨슈머에게 할당.
- 그리고 이 컨슈머는 마지막 오프셋 레코드부터 읽기 시작.
- 이전 컨슈머가 처리 성공한 메시지 오프셋은 커밋이 안 되어 있으니,
- 결국 두 번의 처리가 발생.

#### Reprocessing caused by zombie applications

- 앞의 사례와 비슷한 부분이 있음.
- 애플리케이션이 레코드 배치를 컨슘 한 뒤, 잠시 멈춘다거나 연결이 끊어지면?
- 일정 시간 하트비트 실패하므로, 애플리케이션은 죽었다고 간주되고, 파티션은 새로운 컨슈머에게 할당.
- 컨슈머는 레코드 배치를 다시 읽어 처리.
- 그런데, 죽었다고 간주되었던 애플리케이션이 다시 살아나서 활동을 재개한다면?
- 자신이 죽었다는 사실을 모르는 컨슈머를 가리켜 좀비라 부름.
- 이미 처리된 메시지가 중복으로 발생할 수 있음.

### 8.2.3 How Do Transactions Guarantee Exactly-Once?

먼저, 뼈대 아이디어 소개.

- 간단한 스트림 처리 애플리케이션이 있음.
- 하나의 토픽에서 데이터를 읽고, 처리하고, 다른 토픽에 결과를 씀.
- 정확히-한 번이라 함은 컨슘, 처리, 발행이 원자적으로 수행됨을 의미.
- 부분적 결과는 없어야 함(오프셋만 커밋되고 결과 발행은 없는 것).
- 이를 위해 카프카는 원자적 멀티파티션 쓰기<sup>atomic multipartition writes</sup> 개념을 도입.
- 오프셋 커밋과 결과 발행 모두 파티션에 메시지 쓰기를 동반.
- 다만, 결과는 출력 토픽에 쓰고, 오프셋은 `_consumer_offsets` 토픽에 씀.
- 트랜잭션을 열고, 두 메시지 모두 쓰기를 시도한 뒤, 쓰기가 모두 성공한 경우에 커밋한다면, 정확히-한 번 시맨틱을 얻을 수 있음.
- [Transactions in Apache Kafka](https://www.confluent.io/blog/transactions-apache-kafka/)에서 오프셋 커밋이란, 컨슘된 오프셋을 마킹(오프셋 토픽에 쓰기)하는 것을 가리킴.

아래 그림은 2개의 파티션에 대해 트랜잭션 쓰기를 수행하는 모습. _consumer_offsets에도 커밋.

![](./8.2.3.A%20atomic%20multipartitions%20write.png)

다음으로, 트랜잭셔널 프로듀서 이야기. 재시작에도 같은 `producer.id`를 할당 받는 게 키.

- 트랜잭션과 원자적 멀티파티션 쓰기를 사용하려면, 트랜잭셔널 프로듀서를 이용해야 함.
- 여기서 트랜잭셔널 프로듀서란, `transactional.id`가 설정되고 `initTransactions()`를 사용해 초기화된 카프카 프로듀서를 가리킴.
- 카프카 브로커에 의해 자동 생성되는 `producer.id`와 달리, `transactional.id`는 프로듀서의 설정이고 재시작에도 영속성을 기대.
- `transactional.id`는 사실상 재시작에도 같은 프로듀서를 식별하기 위한 게 주된 역할.
- 브로커는 `transactional.id`를 `producer.id`로 매핑해 둠.
- 만약, 같은 `transactional.id`와 함께 `initTransactions()`을 수행한다면, 프로듀서는 기존과 동일한 `producer.id`를 할당 받음.

좀비 인스턴스 방지 메커니즘도 설명.

- 앞에서, 좀비 인스턴스는 중복을 만든다고 했었음.
- 이를 막기 위해 zombie fencing 사용.
- 좀비 인스턴스가 결과 스트림에 쓰는 것을 방지하는 것.
- 구체적으로, epoch를 사용.
- 트랜잭셔널 프로듀서의 `initTransaction()` 시, `transactional.id`에 대한 epoch 번호가 증가함.
- `transactional.id`가 같더라도 epoch가 현재보다 낮으면 `FencedProducer` 오류와 함께 거절.
- 이를 받은 오래된 프로듀서는 `close()`를 수행해야 함.
- 카프카 2.5 이후 버전에서는, 트랜잭션 메타데이터에 대한 컨슈머 그룹 메타데이터를 지정할 수 있음.
- 이는 서로 다른 트랜잭셔널 ID를 가진 프로듀서가 같은 파티션에 쓰기를 수행하면서도 좀비 인스턴스는 fencing하게 해 줌.

지금까지 프로듀서 쪽 얘기를 했는데, 트랜잭션을 위해서는 컨슈머 측 처리도 필요하다는 이야기.

- 트랜잭션은 대부분 프로듀서 기능.
- 트랜잭셔널 프로듀서 사용, 트랜잭션 시작, 여러 파티션에 레코드 쓰기, 이미 처리한 레코드를 마킹하기 위한 오프셋 발행, 트랜잭션 커밋 또는 중단 모두 프로듀서에서 일어나는 일.
- 그러나 결국 중단된 트랜잭션을 대응하려면, 컨슈머에 적절한 isolation 보장 설정이 있어야 함.

이제 메시지 수신 측 이야기.

- 트랜잭션을 고려한 메시지 읽기를 위해 `isolation.level` 설정 필요.
- `read_committed`로 설정되면, `consumer.poll()` 호출 시 성공적으로 커밋된 트랜잭션 메시지 또는 트랜잭션 없는 메시지를 반환.
- 아직 열려 있거나, 중단된 트랜잭션 메시지는 읽지 않음.
- `isolation.level`의 기본 값인 `read_uncommitted`에서는 모든 레코드가 반환됨.
- 단, 애플리케이션이 트랜잭션의 모든 메시지를 받는 건 아님. 트랜잭션의 묶음 토픽 중 일부만 구독 중일 수 있기 때문.
- 또한, 애플리케이션은 트랜잭션이 언제 시작되고 끝나는지, 어느 트랜잭션의 어느 메시지인지 알 수 없음.

![](./8.2.3.B%20consumers%20read_committed.png)

마지막으로 지연 이야기.

- `read_committed` 모드에서 메시지 읽기 순서 보장을 위해, 트랜잭션이 열려 있는 이후의 메시지는 반환이 안 됨.
...
- 어느 쪽에 속하는지를 알아야 수집 후 서로 다르게 처리 가능.
- 경보를 위한 지표는 당장의 대응을 위한 것이므로 보관 기간이 길지 않아도 됨.
- 이번 시스템 다운을 대응하면 되는 거지, 지난번 다운에 대응할 것은 아니기 때문.
- 우리에게 문제를 알려주는 자동화된 시스템들이 수집하는 것이 보통(물론 사람이 할 수도).
- 진짜 중요한 문제만 알려줄 수 있도록 목표를 명확히 하는 게 좋음.
- 한편, 디버깅을 위한 데이터는 보관 기간이 길어야 함.
- 상대적으로 좀 더 주관적이거나, 카프카 애플리케이션의 데이터일 수도.
- 항상 수집할 필요는 없고 필요할 때 얻을 수만 있으면 됨.

자동화가 목적인지, 사람이 볼 것인지?

- 자동화가 목표일 경우 지표는 매우 명확해야 함.
- 그리고 아주 세세하게 대량 데이터 수집도 괜찮음.
- 하지만 사람이 봐야 할 경우 너무 많은 데이터에 압도당할 수도.
- 경보 피로감<sup>alert fatigue</sup>에 빠져 경보를 무시하는 문제로 이어지기도.
- 자동차의 공연비 조정을 위해 컴퓨터는 공기 밀도나 연료 등 엔진 관련 여러 측정값이 필요하지만, 이게 일반 운전자에게는 버거운 것과 유사.

### 1.3.2.1 서비스 수준 정의

- 일반적인 SLI, SLO, SLA 이야기.
- SLO를 서비스 수준 한계<sup>service-level threshold</sup>(SLT)라고도 부른다는 건 처음 봄.

### 13.2.3 경보에 SLO를 사용하기

- 재밌는 얘기가 있어 기록.
- SLO는 직접적인 경보 기준으로 사용하기 매우 어려움.
- SLO는 장기간(예컨대 1주일) 기준으로 할 때 제일 좋음.
- 하지만 SLO 경보가 울릴 때쯤이면 이미 늦음(이미 SLO를 달성하는 게 물 건너감).
- 그래서 SLO 경보에 사용하는 가장 좋은 방법은 소진율<sup>burn rate</sup>을 보는 거라고.
- 예를 들어, 카프카 클러스터가 매주 100만 개의 요청을 받음.
- SLO는 전체 요청의 99.9%가 10ms 안에 응답하는 것이라고 해보자.
- 즉, 매주 1천 개까지는 좀 더 응답이 늦어도 됨.
- 한 시간에 한 번 이런 요청이 발생한다면, 일요일부터 다음 주 토요일까지 1주일에 168개의 지연 응답이 생김.
- 이게 소진율. 1주일에 100만 개의 요청이 주어지는 상황이라면 1시간에 1개의 응답 지연은 시간당 소진율이 0.1%가 됨.
- 하지만 수요일 오후 2시, 소진율이 시간당 2%가 넘어선다면, 이 추세로는 금요일 점심이면 에러 예산을 다 쓰게 됨.
- 따라서 즉시 뭔가를 대응해야 함.
...
- `record-error-rate` 속성은 반드시 경보 설정해야 함.
- 이 지표는 언제나 0이어야 하고, 이보다 크면 프로듀서가 브로커로 메시지를 보내는 와중에 누수가 있음을 의미.
- 프로듀서가 백오프를 해 가며 사전 설정된 수만큼 재시도 후 최종 실패하면 메시지는 폐기.
- `record-retry-rate` 속성을 추가할 수도 있으나 재시도 자체는 정상 경우도 있어 크게 중요치는 않음.
- `request-latency-avg`도 경보 설정이 필요.
- 이는 브로커가 쓰기 요청을 받을 때까지 걸린 평균 시간.
- 정상 상태일 경우의 값을 찾은 뒤 이 값보다 높은 값을 임계치로 설정하면 됨.
- 프로듀서가 전송하는 메시지 트래픽도 미리 알아두는 게 좋음.
  - `outgoing-byte-rate`: 전송 메시지의 절대 크기를 초당 바이트 형태로 표현.
  - `record-send-rate`: 초당 전송되는 메시지의 수.
  - `request-rate`: 브로커로 전달되는 쓰기 요청의 수.
  - 하나의 배치는 1개 이상의 메시지를 포함할 수 있으므로 메시지 수와 요청 수는 다를 수 있음.
- 메시지, 요청, 배치 크기를 나타내는 지표도 있음.
  - `request-size-avg`
  - `batch-size-avg`
  - `record-size-avg`
  - `records-per-request-avg`
- 마지막 권장 속성은 `record-queue-time-avg`.
- 애플리케이션 프로듀서 내부 버퍼(queue)에 메시지가 들어가서 전송되기 전까지 머무른 평균 시간(ms).
- 보통 batch.size 설정 크기만큼 배치가 채워지거나, 마지막 배치가 전송된 후 linger.ms에 지정된 시간이 경과해야 메시지가 전송.

브로커별, 토픽별 지표

- 프로듀서의 종합적 상황 말고 브로커나 토픽에 관한 정보도 있음.
- 디버깅 시 유리한 정보.
- 특정 브로커의 `request-latency-avg` 값이 안정적이지 않으면 브로커 연결 문제일 수도.
- `outgoing-byte-rate`나 `request-latency-avg`는 브로커가 리더를 맡고 있는 파티션이 무엇이냐에 따라 달라지는 경향이 있어 조금 다르게 모니터링.
- 토픽별 지표는 브로커보다는 약간 더 중요.
- 단, 프로듀서가 2개 이상의 토픽에 쓰고 있을 때만 유용.
- 그리고 너무 많은 토픽에 쓰기 작업을 수행하고 있다면 지표를 살피긴 어려움.
- 이 역시 브로커 지표와 마찬가지로 디버깅에 필요한 것.

### 13.4.2 컨슈머 지표

읽기 매니저 지표

- 컨슈머의 중요 지표들은 읽기 매니저<sup>fetch manager</sup> 빈에 들어 있다고.
- 그래서 컨슈머에서 종합 지표들은 상대적으로 덜 중요.
- 컨슈머 종합 지표에는 저수준 네트워크 작업 관련 지표들.
- 읽기 매니저 빈에는 바이트, 요청, 레코드에 대한 지표들.
- 그리고 컨슈머 지표들에는 경보 설정에는 그리 유용치 않다고 함.
- 딱 하나 `fetch-latency-avg`는 경보 설정에 사용.
- 브로커로 읽기 요청을 보내는 데 걸리는 시간을 가리킴.
- 다만, fetch.min.bytes와 fetch.max.wait.ms의 영향으로 시간 지표가 일정치 않음에 유의.
- 정기적으로 많은 메시지가 들어오는 토픽에 대해서는 사용할 가치가 있음.
- 컨슈머 클라이언트가 얼마나 많은 메시지 트래픽을 처리중인지 알려면 `bytes-consumed-rate` 혹은 `records-consumed-rate` 사용.
- 다만, 프로듀서의 상태에 따라 값이 영향 받으므로 경보로 사용하기에는 부적합.
- `fetch-rate`, `fetch-size-avg`, `records-per-request-avg`를 통해 바이트, 메시지, 요청 사이의 관계도 파악.

브로커별, 토픽별 지표

- 프로듀서에서와 마찬가지로 브로커별, 토픽별 지표도 제공.
- 문제를 디버깅하는데 도움 될 수 있음.

컨슈머 코디네이터 지표

- 컨슈머 클라이언트는 일반적으로 컨슈머 그룹의 일원으로 함께 동작.
- 컨슈머 그룹은 그룹 코디네이션 작업을 수행하고, 여기에는 그룹 멤버 합류나 그룹 멤버십 유지를 위해 브로커에 하트비트 전송하는 것도 포함.
- 이들을 모니터링하는데 필요한 몇 가지 지표들을 제공.
- 컨슈머에 생길 수 있는 코디네이터 관련 가장 큰 문제는, 컨슈머 그룹 동기화로 인한 읽기 작업 일시 중지.
- 어느 컨슈머 인스턴스가 어느 파티션을 담당할지 합의할 때 발생하는 절차.
- 읽는 파티션 수에 따라 꽤 시간이 걸릴 수도.
- 동기화 작업에 들어간 평균 시간은 `sync-time-avg`나 `sync-rate`로 알 수 있음.
- 안정적인 상황이라면 대부분의 시간 동안 0을 유지.
- 컨슈머 코디네이터는 오프셋 커밋에 걸린 평균 시간을 `commit-latency-avg`를 통해 제공.
- 정상적인 경우의 값이 얼마인지 보고 적절한 경보를 설정해야 함.
- 마지막으로, `assigned-partitions` 메트릭 수집도 유용.
- 같은 그룹의 다른 컨슈머 클라이언트와 비교했을 때 너무 많은 부하가 몰리지 않았는지 확인에 필요.

### 13.4.3 쿼터

- 카프카는 한 클라이언트의 전체 클러스터 독차지를 방지하고자 클라이언트 요청을 스로틀링.
- 프로듀서와 컨슈머 측 모두 설정 가능하고, 클라이언트의 ID와 각 브로커에 허용된 트래픽 형태로 표시.
- 스로틀링 한다고 에러 코드를 응답하지는 않기에, 스로틀링을 알려면 지표를 모니터링해야 함.

## 13.5 랙 모니터링

- 컨슈머에 있어 가장 중요한 모니터링이라고 함.
- 랙 = 컨슈머가 마지막으로 읽고 처리한 메시지 번호 - 프로듀서가 특정 파티션에 마지막으로 쓴 메시지 번호
- 시간이 메시지 수로 측정하는 것.
- 컨슈머 지표에서 함께 다루지 않은 이뉴는, 외부 모니터링을 사용하는 게 낫기 때문.
- 컨슈머 클라이언트에 있는 랙 지표는 가장 지연 심한 하나의 파티션만 나타냄.
- 따라서 컨슈머가 얼만큼 뒤쳐져 있는지는 정확히 알 수 없음.
- 링크드인에서 개발된 Burrow 도구를 소개.
- 클러스터 내 모든 컨슈머 그룹의 랙 정보 가져온 뒤, 각 그룹이 제대로 동작하는지 뒤쳐지는지 일시 중지되었는지 완전히 중지되었는지를 계산해서 보여줌.
- 이런 도구 사용을 강력하게 권장.
