...
- 앞 장에서는 소프트웨어 시스템의 경제 가치를 할인된 미래 현금 흐름의 합으로 모델링.
  - 더 많이, 더 일찍, 더 높은 확률로 돈 벌기.
  - 더 적게, 더 늦게, 더 적은 확률로 돈 쓰기.
- 소프트웨어 설계는 그러나 골디락스 세상.
- 너무 과하지도 않게, 너무 이르지도 않게, 너무 적지도 않게, 너무 늦지도 않게 설계해야 함.
- 그리고 여기서 나아가, 상충되는 가치인, 옵션도 고려해야 함.
- 저자가 옵션을 경험하면서 얻은 교훈은 아래와 같다고 함.

```
1. "다음에 어떤 동작을 구현할 수 있을까?"는 구현하기 전에도 이미 그 자체로 가치가 있음. 내가 받는 돈은 이미 내가 한 것이 아니라, 내가 다음에 뭘 할지에 대한 보상.
2. "다음에 어떤 동작을 구현할 수 있을까?"의 가치는 포트폴리오에 기능 목록이 더 많을수록 커짐. 포트폴리오의 크기를 늘리면, 더 많은 가치를 창출한 것.
3. "다음에 어떤 동작을 구현할 수 있을까?"의 가치는 포트폴리오에 있는 기능들이 더 가치 있을 때 더욱 커짐.
4. 어떤 옵션을 구현할지 열려 있는 한, 어떤 기능이 제일 가치 있는지는 중요치 않음.
5. 그리고 가장 중요한 건, 가치에 대한 예측이 불확실할수록, 옵션의 가치는 더 큼. 변화를 수용하면 가치 창출 극대화.
```
