...
- 목표를 이해하는 게 중요. 목표를 알면 거절해야 하는 프로젝트를 발견하기도.
...
- 또한 제약을 알아야 도달 가능한 목표 설정이 가능.
- 모르는 부분이나 위험한 부분이 어디인지 찾고 리스크를 관리. 자주 릴리즈하며 피드백을 받는 것이 도움이 됨.
- 기존의 프로젝트라면 무작정 뛰어드는 대신, 다양한 대화를 자주 나누며 확인하기(이전부터 있던 것을 존중하라).

#### 5.2.3 프로젝트 공식 구조 설정

- 각 분야별 리더들과 명확한 역할 정의.
- 인재 채용에도 투자. 좋은 기술적 판단력, 낙관적이고 갈등 해결에 능한 것, 의사소통 역량, 좋은 태도 등이 중요.
- 프로젝트 범위를 최대한 세부적으로 쪼개면, 점진적으로 나아가며 동기부여를 얻을 수 있고, 일을 뒤로 미루지 않게 하는 효과.
- 작은 단위로 배포하며 피드백을 얻어서, 일정에 대한 충분한 정보를 계속 수집하고 추정해야 함.

뒷 내용들은 특별한 게 없어 기록 생략. RFC 얘기만 정리.
