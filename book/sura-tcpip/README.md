...
- 유선 LAN에서는 네트워크 허브라는 장비에 케이블을 연결해서 네트워크를 구성.
- 집에서는 인터넷 업체에서 제공하는 초고속 인터넷 라우터를 이용하는데 여기에 네트워크 허브 기능이 내장됨.
- 이더넷을 사용하는 경우는 여러 네트워크 토폴로지 중 스타형에 해당(허브에 여러 호스트가 연결된 구성).
- L2 스위치(스위칭 허브)는 가장 많이 사용되는 네트워크 중계 기기.
- L2는 각 포트마다 서로 다른 호스트가 연결 되어 있고,
- 포트와 호스트의 MAC을 매핑하며 통신회선이 독립적이므로,
- 통신이 동시에 이뤄진다고 하더라도 패킷 충돌 피할 수 있음.
- 브로드캐스트 도메인 얘기도 언급.
- 수신지의 주소가 브로드캐스트 어드레스일 때 데이터가 전달되는 범위를 가리킴.
- L2 스위치로 네트워크를 구성한 경우 네트워크 전체가 브로드캐스트 도메인임.
- 만약, 네트워크에 연결된 호스트 수가 많으면 DHCP 등의 브로드캐스트 통신 시 대량 패킷 발생으로 부하 생길 수 있음.
- L3 스위치는 VLAN 기능으로 LAN을 몇 개의 가상 네트워크로 분할할 수 있음.
- 위에서 얘기한 브로드캐스트 통신의 부하를 제한할 수 있는 것.

## 무선 LAN

- 무선 LAN은 전파를 이용해서 네트워크를 구성.
- 정식 규격명은 IEEE 802.11이라고 함.
- CSMA/CA<sup>Carrie Sense Multiple Access with Collision Avoidance</sup> 방식을 사용.
- 다른 통신 장비의 전파 발신 여부를 확인 후 겹치지 않게 통신을 시작하는 것.
- 무선 LAN의 프레임은 프리앰블(프레임 시작), PLCP 헤더(신호 변조 방식), MAC1, MAC2, MAC3, 일련번호(데이터 분할된 경우 일련번호), MAC4, 데이터 본체, FCS(프레임의 끝)로 이뤄짐.
- MAC 주소가 4개나 할당된 것은 무선→무선, 무선→유선, 유선→무선, 유선→무선→유선 방식을 서로 다르게 표현하기 위함.

## ARP

- 이더넷이나 무선 LAN으로 데이터 전송을 위해 수신 MAC 어드레스를 알아야 함.
- 이 때 ARP<sup>Address Resolution Protocol</sup>를 사용.
- 송신 장비가 요청 패킷에 수신 IP를 설정 후 네트워크 전체에 브로드 캐스팅.
- 이 IP에 맞는 수신 장비는 자신의 MAC 정보를 응답 패킷에 설정하여 응답.
- 이런 식으로 IP를 통해 MAC을 알아내는 과정을 거침.
- 서브넷 마스크가 다른 호스트에 대해서는 프락시 ARP를 이용.
- 서로 다른 네트워크를 프락시 ARP 기능을 가진 라우터 등이 MAC 어드레스를 대신 응답하는 것.
