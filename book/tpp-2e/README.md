...
액터와 프로세스는 공유 메모리에 대한 접근 동기화 부담 없이, 동시성을 구현할 수 있는 한 가지 방식. 일단 각각의 기본 개념 언급.

**actor**

- 독립적인 가상 프로세서.
- 로컬(그리고 private) 상태를 가짐.
- 각 액터는 메일 박스를 가짐.
- 메시지가 메일 박스에 생기고 액터가 유휴 상태라면,
- 메시지를 갖고 들어와서 처리를 시작함.
- 처리가 끝나면 메일 박스의 다른 메시지를 처리하고,
- 처리할 메시지가 없다면 다시 유휴 상태로 돌입.

**process**

- 좀 더 범용적인 가상 프로세서.
- 동시성을 돕기 위해 종종 OS에서 구현됨.
- 액터처럼 동작하도록 제한 될 수 있음.

### Actors Can Only Be Concurrent

액터의 개념만으로는 부족한 내용에 대해 좀 더 설명.

- 통제되는 것은 없음. 다음에 일어날 일을 스케쥴링 X. 데이터 전송 조율 X.
- 시스템의 상태는 오직 메시지 내부에 담기거나 각 액터의 로컬 상태 뿐. 메시지는 수신자만이 읽을 수 있고, 액터의 로컬 상태는 외부에서 접근 불가.
- 모든 메시지는 단방향. 답변 개념이 없음. 액터가 응답하고 싶다면, 메시지에 메일박스 주소를 담아서 그곳으로 새로운 메시지를 보내도록 해야 함.
- 액터는 각 메시지를 완료 할 때까지 처리하고, 한 번에 한 메시지만을 처리.

결과적으로 액터는 동시에, 비동기적으로, 공유하는 것 없이 실행됨.
...
- 액터 모델에서는 동시성을 다루는 코드가 필요 없음.
- 공유된 상태가 없기 때문.
- 그리고 액터는 자신이 받은 메시지를 기반으로 동작하기에,
- 이거 하고 저거 하라는 식의 순서가 명시되지 않음.
- 마지막으로, 단일 프로세서나, 멀티 코어나, 멀티 네트워킹 머신에서도 잘 동작.

### Erlang Sets the Stage

- Erlang 언어와 런타임은 액터 구현체의 좋은 예시.
- Erlang은 액터 프로세스를 호출함.
- 그런데 이 프로세스는 보통의 OS 프로세스가 아님.
- Erlang의 프로세스는 경량이며 메시지를 보내는 것으로 서로 통신.
- 서로 격리되어 있으며 아무런 상태도 공유하지 않음.
- 여기에 더해 Erlang 런타임은 *supervision* 시스템도 제공.
- 프로세스의 생애주기를 관리하고, 실패한 프로세스를 재시작해 주기도.
- 또한 핫<sup>hot</sup>-코드 로딩도 지원.
- 시스템을 멈추지 않고도 실행 중인 시스템의 코드를 교체하는 것.
- Erlang은 가용성도 매우 높음(99.9999999 언급).

## Topic 36. Blackboards

- 칠판의 이점 이야기.
- 이는 자유방임주의<sup>laissez faire</sup> 동시성의 한 형태.
- 칠판을 사용하는 사람들은, 시간이나 다른 사용자 등과 독립적.
- 추가된 정보가 있으면 칠판에 그냥 추가하면 됨.
- 칠판은 조금씩 결론으로 이끌어 줄 것.
- 나머지 내용은 기록 생략.

# Chapter 7. While You Are Coding

- 프로젝트가 코딩 단계에 들어서면,
- 이는 단지 기계적인 작업이고 설계를 실행가능한 문장으로 바꾸는 것이라는 얘기들이 있음.
- 하지만 이런 생각은 프로젝트 실패의 이유.
- 코딩은 기계적이지 않음.
- 코딩에는 매 순간 결정해야 하는 것들이 있음.
- 신중한 고려와 판단이 없다면 애플리케이션의 생명은 길지 않을 것.

## Topic 37. Listen to Your Lizard Brain

- 본능에 대한 이야기.
- 타고난 것이거나 반복에 의해 학습된 것.
- 이들의 공통점은 no words.
- 생각이 아니라 느끼게 함.
- 이 본능의 좋은 점을 알고 잘 활용하는 방법 이야기.

### Fear of the Blank Page

- 아무 것도 없는 상태에서 최초 커밋을 남기는 것을 두려워 할 때가 많음.
- 하지만 이는 무언가 말해주고 있는 신호.
- 개발자로써 이런 저런 경험을 많이 했기 때문일 것.
- 이런 신호에 귀를 기울이고 생각할 시간을 갖는 것이 도움이 됨.

### Fighting Yourself

- 어떤 때는 코드 작성이 매우 수월할 때가 있음.
- 하지만 어떤 때는 매우 더디고 자꾸 뒤로 가기도 함.
- 이럴 때 끈기 있게 한 걸음씩 꾸역 꾸역 나아가는 것은 X.
- 이는 우리에게 코드가 무언가 말해주고 있는 것.
- 구조나 설계가 잘못된 것일 수 있음.
- 잘못된 문제를 풀고 있는 걸 수도.
- 버그를 만들고 있는 중일 수도.
- 어느 상황이든 우리의 본능이 코드로부터 피드백을 주고 있는 것.
- 의식적으로 이를 들으려 노력해야 함.
...
- 본능의 소리를 듣는 방법 이야기.
- 일단 하던 것을 멈춤.
- 키보드를 멀리하고 잠시 머리를 쉬어라.
- 산택, 점심, 수다 등을 하기.
- 그러면 무의식 중에 있던 것이 의식 위로 떠오를지도.
- 만약 이 방식이 작동하지 않는다면,
- 이슈를 바깥으로 끌어내 볼 것.
- 동료에게 설명하거나, [고무 오리](https://en.wikipedia.org/wiki/Rubber_duck_debugging)를 이용.
- 동료의 조언을 구할 수도, 스스로 설명하면서 아이디어가 떠오를 수도.
- 그래도 안 되면 뇌에게 너가 하려는 것은 중요치 않다고 말하기.
- 이를 프로토타이핑(바로 뒤에서 다루는 내용)을 통해 수행.

### It's Playtime!

- 생각을 돕는 프로토타이핑 이야기.
- "나는 프로토타입핑 중이다"라고 모니터 옆에 적기.
- 프로토타입이 실패해도 되고, 실패하지 않는다고 해도 버려질 것임을 되내이기.
- 빈 에디터 버퍼에, 무엇을 하려고 하는지 한 문장으로 적기.
- 코딩하고 지우고 수정하고 다시 작성하고 반복.
- 그러는 와중에 무엇을 하려는지 깨닫게 됨.

## Topic 38. Programming by Coincidence

- 거짓 결론에 유의해야 함.
- 우연에 의한 프로그래밍(행운이나 우연한 성공에 의존)을 피해야 함.
- 대신, 의도적인 프로그래밍을 선호해야 함.

### How to Program by Coincidence

- 코드를 작성해 보고 테스트 하니 성공.
- 하지만 왜 제대로 동작하는지는 이해 못 함.
- 몇 주가 지난 뒤 갑자기 코드가 동작을 멈춤.
- 왜 동작하는지를 모르기에 왜 실패하는지도 모름.
- 우연한 성공.

#### Accidents of Implementation

- 잘못된 데이터를 어떤 루틴에 입력했는데, 
- 그 루틴이 이 예외 케이스에 대응이 되어 있지 않아서,
- 마치 루틴이 성공했을 때의 결과값을 반환.
- 하지만 이 문제가 고쳐지면,
- 이 잘못된 데이터로 루틴을 사용하던 곳에 영향.
- 잘못된 순서나 잘못된 문맥으로 호출하는 것도 비슷한 문제.
...
- 위 코드는 GUI 프레임워크를 이용해서 화면에 무언가를 출력하려는 코드.
- 하지만 이 루틴들은 이런 식으로 호출될 것이라고 가정하지 못했기에,
- 정상적으로 동작하는 것처럼 보일지 몰라도 순전히 우연.
- 언제든 깨질 수 있는 코드.
- 사용하려는 루틴에서 문서화 된 내용에만 의존해야 함.
- 혹시라도 그럴 수 없다면 당신의 가정을 잘 문서화.

#### Close Enough Isn't

- 여러 시스템 데이터를 수집하려는데,
- 각 시스템은 로컬 타임존을 사용했으며,
- 타임존의 충돌과 일광 절약 시간제 정책의 불일치로,
- 결과 값이 정확하지 않은 문제가 있었는데,
- 마침 그 결과가 1만 차이나는 상황이었기에,
- 이를 사용하는 함수가 1을 더하거나 빼는 것으로 문제를 해결하곤 함.
- 여기서 "오직" 1만 달라진다는 것은 우연.
- 깊고 근본적인 결함을 가릴 뿐.
- 시간이 지날수록 +1과 -1 문장들로 점점 유지보수 하기 어려워짐.

#### Phantom Patterns

- 인간은 패턴과 원인을 보도록 되어 있음.
- 그것이 우연인 경우에도 패턴으로 인식.
- 러시아는 200년 동안 대머리와 머리 있는 사람이 번갈아 가며 대통령을 했다고 함.
- 복권이나 도박, 주식 등의 시스템은 이 반복에 의존해서 도메인을 구현했다고.
- 1,000개의 요청마다 에러가 로그에 남는 것은, 단순한 버그때문일 수도, 경합 조건에 의한 것일 수도 있음.
- 로컬에선 괜찮은데 테스트 장비에서는 에러가 난다면, 환경의 차이일 수도 있고, 단지 우연의 일치일 수도.
- 가정하지 말고 증명하라.
...
- "컨텍스트의 우연"도 일어날 수 있음.
- 만약 어떤 유틸리티 모듈을 만들고 있다면,
- 지금 애플리케이션이 GUI 환경이므로 GUI 환경만 고려하면 되는 것일지,
- 영어 사용자 등 현재는 보장되지 않는 어떤 또 다른 것들을 가정해야 할지?
- 인터넷에 돌아다니는 코드나 오픈소스를 가져왔을 때,
- 그것이 사용됐던 환경과 지금의 환경이 같을 것이라고 보장할 수 있는가?
...
- 우연성은 모든 레벨(요구사항 도출부터 테스트까지)에서 오도될 수 있음.
- 테스팅 단계가 특히 거짓 인과성과 우연성의 결과로 만연한 곳.
- X과 Y를 일으킨다고 가정하기 쉽지만, 가정하지 말고, 증명하라.
- 모든 레벨에서 사람들은 마음속에 많은 가정을 갖고 있음.
- 하지만 이 가정은 문서화 되어 있지도 않고, 개발자들간의 충돌이 일어나는 부분.
- 확고한 사실에 기반하지 않는 모든 가정을 금하라.

> At all levels, people operate with many assumptions in mind.

## How to Program Deliberately

불필요한 코드 작성을 줄이고, 에러를 가능한 초기 개발 단계에 발견하고 수정하고, 시작부터 적은 에러를 만들고자 한다면, 의식적인 프로그램 작성이 도움이 됨.

1. 본인이 무엇을 하고 있는지 인지하기. boiled frog 다시 언급.
2. 후배 개발자에게 코드를 상세히 설명할 수 있어야 함.
3. 애플리케이션 전체를 이해하며 코드를 작성하고, 충분한 이해가 있는 기술을 사용.
4. 계획을 세우고 나아가라.
5. 신뢰할 수 있는 것에만 의존. 가정에는 의존 X.
6. 가정한 것이 있다면 문서화(Design by Contract). 스스로의 가정을 명확히 할 수 있고, 다른 이들과 커뮤니케이션 하는 데도 도움.
7. 코드뿐만 아니라 가정도 테스트하라. 추측하지 말고 해보라. 단언문이 도움이 됨.
8. 우선순위를 정하고 중요한 것에 시간을 사용.
9. 역사의 노예가 되지 말라. 현재 코드가 미래를 결정하게 하지 말라. 적절치 않으면 교체.

## Topic 40. Refactoring

- 프로그램이 진화하면, 초기의 결정을 재고하고, 일정량의 코드를 재작업 하는 일은 필수.
- 이 과정은 자연스러운 일. 코드는 정적인 것이 아니라 진화하는 유기체.
- 마틴 파울러는 리팩토링 책에서 리팩토링을 아래와 같이 정의.

> disciplined techinque for restructuring an existing body of code, altering its internal structure without changing its external behavior.

- 이 정의에서 중요한 부분은 2가지.
- 먼저, 이 활동은 숙련도가 필요.
- 다음으로, 외부 행위가 바뀌지 않음.
- 리팩토링은 매일 매일 이루어지는,
- 적은 리스크를 가진 작은 단계들의 연속인,
- 잡초 제거와 같은 작업임.
- 아무렇게나 막 하는 것이 아니고,
- 전체 코드베이스를 재작성하는 것도 아님.
- 목표를 정하고 정밀한 접근법이 필요.
- 외부 행위가 바뀌지 않음을 보장하기 위해,
- 자동화 된 테스트 거증이 필요.

### Real-World Complications

- 리팩토링을 하지 않는 핑계로 시간 압박을 많이 사용.
- 하지만, 지금 리팩토링을 못한다면, 나중엔 더 많은 비용이 발생.
- 예컨대 더 많은 의존성이 일어나 있을 것.
- 리팩토링도 다른 것과 마찬가지로 이슈가 작을 때 수행하기 쉬움.
- 코드를 작성하는 작업의 한 부분으로 인식.
- 리팩토링에 1주일이 필요하다면 그것은 리팩토링이라기보다 전체 재작성에 가까움.
- 그런 작업은 당장엔 할 수 없고 계획을 세워서 해야 하는 일.
- 그리고 미리 계획을 공유하고 어떤 영향을 미칠지도 함께 알릴 것.

### How Do You Refactor?

마틴 파울러는 리팩토링이 순작용을 하기 위한 몇 가지 팁을 제공.

1. 리팩토링과 기능 변경(추가)은 동시에 X.
2. 리팩토링을 시작하기 전에 좋은 테스트가 있어야 함. 그리고 이 테스트를 가능한 자주 실행. 이를 통해 변경이 무언가 깨뜨리는 것이 있는지를 확인.
3. 짧고 의도적인 단계를 밟을 것. 필드를 다른 클래스로 옮기고, 메서드를 나누고, 변수를 리네임, ... 이 작은 단계를 밟음으로써 계속되는 디버깅<sup>prolonged debugging</sup>을 피할 수 있음.

## Topic 41. Test to Code

- 왜 테스트를 하냐고 물으면,
- 코드가 제대로 동작하는지 확인하기 위함이라고 하는데,
- 저자는 여기에 동의하지 않음.
- 테스팅에 관해 중요한 것은 그럼 무엇일까?
- 테스트의 주요 이점은 그것을 실행할 때가 아니라,
- 그것을 작성할 때 우리를 생각하게 만든다는 것에 있음.
...
- 월요일 아침 새로운 코드를 작성하려고 자리에 앉음.
- DB에 질의해서 이번 주에 비디오를 10개 이상 본 사용자 목록을 받아야 함.
- 이제 에디터를 열고 쿼리를 수행하는 함수를 작성하려고 함.
...
- 잠깐! 지금 하려고 하는 것이 좋은지 아닌지를 어떻게 알 수 있는가?
- 알 수 없다가 답. 아무도 알 수 없음.
- 하지만 테스트에 대해 생각하는 것이 가능성을 높여줌.
- 일단, 함수를 다 작성했고 테스트를 작성할 차례라고 상상.
- 먼저 테스트 데이터가 필요할텐데,
- 우리는 DB를 다룰 때 프레임워크를 사용하지 않으므로,
- 함수에 DB 인스턴스를 넘겨야 함을 알 수 있음.
- 테스트 하는 동안 이 인스턴스를 통해 데이터 조작 가능.

```rb
def return_avid_users(db) do
```

- 다음으로, 어떻게 테스트 데이터를 생성할 수 있을지 생각.
- DB 스키마를 살펴보니 `opened_video`와 `completed_video` 필드가 있음.
- 아직은 어떤 필드를 사용해야 할지 요구사항이 명확치 않으니,
- 필드 명을 파라미터로 넘기는 것으로 미래 변경에 대응.

```rb
def return_avid_users(db, qualifying_field_name) do
```

- 지금까지 테스트에 대한 생각으로 작업을 시작했고,
- 작성된 코드는 없으며,
- API 변경이 필요한 2가지 발견을 했음.

### Tests Drive Coding

- 테스트는 구현 관점이 아닌,
- 코드 사용자 관점에서 생각하게 해 줌.
- 이전 예제에서는 이를 통해 결합도 줄어듦.
- 전역이 아닌 DB 커넥션을 파라미터로 넘긴 것.
- 더불어 유연성이 높아짐.
- 필드명을 파라미터로 받은 것.
...
- 테스트는 코딩을 가이드하는 주요한 피드백.
- 결합도 높은 코드는 테스트하기 어려움.
- 메서드를 실행하기에 앞서 모든 환경을 준비해야 하기 때문.
- 따라서 테스트 가능하게 만드는 노력은 낮은 결합도를 가져옴.
- 그리고 테스트를 작성하려면 먼저 그것을 이해해야 함.
- 이런 이해를 통해 좀 더 효율적인 구조(덕지덕지가 아닌) 등을 만들어 낼 가능성이 높음.

### TDD: You Need to Know Where You're Going

- 전체 문제를 이해할 수 없을 때 작은 단계를 밟아가며 한 번에 한 가지를 테스트하라고 TDD에서 이야기.
- 하지만 이는 잘못된 길로 이끌 수 있음.
- 코딩하고 있는 진짜 이유를 무시한 채,
- 쉬운 문제를 끊임없이 다듬어 가는 데만 집중하게 만들 수 있기 때문.
- 테스트가 분명 개발을 주도하는 데 도움이 되는 것은 맞음.
- 그러나 목적지를 분명하게 생각하지 않으면 쳇바퀴에 갇힐 것.

### Back to the Code

- 칩들은 생성할 때뿐만 아니라, 설치할 때, 배포할 때도 테스트할 수 있게 설계됐다고 함.
- 좀 더 복잡한 것들은 BIST(Built-In Self Test) 기능을 갖고 있으며,
- 내부적으로 진단을 실행할 수 있게 해 준다고 함.
- 그 외에 TAM(Test Access Mechanism) 이야기도.
- 이 역시 테스트 할 수 있게 도와주는 것.
- 소프트웨어도 마찬가지로, 시작부터 중간 중간 테스트 할 수 있게 하는 것이 중요.

### Unit Testing

- 하드웨어에서의 칩 수준 테스팅은 소프트웨어에서의 단위 테스팅과 유사.
- 각 모듈은 서로 격리되어 행위를 검증.
...
- 최고의 테스트 집합이라 하더라도 모든 버그를 잡을 순 없음.
- 프로덕션 환경은 다르기 때문.
- 따라서 배포 된 후에도 종종 코드를 테스트해야 할 경우가 있음.
- 그러나 이 환경에 디버거 같은 수단은 없음.
- 결국 로그 파일이 이를 돕는 도구.
- 또 다른 도구는 핫키나 매직 URL.
- 핫키를 누르거나 매직 URL을 입력하면,
- 진단 제어 윈도우 팝업이 뜨고 이를 통해 상태 메시지 등을 확인.
- 이는 일반적으로는 사용자에게 보여지지는 않음.
- 하지만 헬프 데스크에겐 매우 유용.

### A Culture of Testing

- 테스트는 중요.
- 테스트를 하는 방법으로 3가지 이야기.
- Test First
- Test During
- Test Never
- 가장 좋은 건 Test First.
- 이는 테스팅을 보장해 주기 때문.
- 하지만 이것이 어렵거나 유용하지 않을 경우 Test During이 대안.
- 코드를 좀 작성하고, 조금 만지작 하다가, 테스트를 작성하고, 또 다음 단계로 가는 것.
- 최악은 Test Later. 이는 사실상 Test Never.
- 테스팅의 문화는 모든 테스트가 항상 성공하는 것을 가리킴.
- 테스트 코드를 프로덕션 코드처럼 여기자.
- 결합도 낮추고, 깨끗하고, 강건하게 유지.
- 신뢰할 수 없는 것에는 의존 X.
...
- 단위 테스트를 작성할 땐 보통 자신의 지식에 기반함.
- 알고 있는 것으로 구현했고, 알고 있는 것에 한해서만 테스트를 작성.
- 하지만 이 지식이 잘못된 가정이라거나 범위가 좁다면 문제가 될 수 있음.
- 이 문제를 해결하는 한 가지 방법은 테스트를 다른 사람이 작성하는 것.
...
- 기능이 제대로 동작함을 확인했다면 이는 90%의 작업이 된 것.
- 이제 코드가 잘못될 수 있는 가능성을 조사하고 이를 테스트 코드로 녹여야 함.
...
- 이름 안에 무엇이 담겨 있을까?
- 프로그래밍에 있어서는 "모든 것!"
- 애플리케이션, 서브시스템, 모듈, 함수, 변수 등.
- 끊임 없이 새로운 걸 만들고 이름을 부여.
- 그리고 이 이름은 매우 매우 중요.
- 이름이 그것의 의도와 생각을 많이 드러내기 때문.
- 이름은 그것의 역할에 따라 지어져야 함.
- 따라서 뭔가를 만들기 전에 잠시 멈추고,
- "이걸 만드는 이유가 뭐였지?"를 생각해야 함.
- 이는 당면한 문제 해결에서 벗어나, 더 큰 그림을 보게 해 줌.
- 이름이 이렇게 중요한 데에는 과학적 근거가 있음.
- 두뇌는 단어를 정말 빠르게 읽고 이해할 수 있다고 함.
- [스투르프 효과](https://en.wikipedia.org/wiki/Stroop_effect) 함께 참고.

다음으로, 이름 관련된 코드 예제 소개.

```js
let user = authenticate(credentials)
```

- 위 예제는 쥬얼리를 파는 사람이 인증하는 코드.
- `user` 대신 `customer`나 `buyer`가 어떨까?
- 이를 통해 코드 작성 시,
- 이 사람이 무엇을 하려는지,
- 우리에겐 어떤 의미인지를,
- 계속 상기시켜 줄 수 있음.
...
- 여기선 2가지.
- 일단, `deductPercent`는 무엇을 하는지만 알려줄 뿐,
- 왜 이것을 하는지는 드러내지 못함.
- 다음으로, `amount` 파라미터는 너무나 모호.
- 절대 값인가 혹은 퍼센티지인가?
- 그래서 아래와 같이 제안.
...
- 이제 이름이 좀 더 의도를 드러냄.
- 더불어, 파라미터 타입을 `Percentage`로 바꾸어 좀 더 명시적.
- `double`일 때는 이 값이 0에서 100 사이인지 0.0에서 1.0 사이인지 알 수가 없음.
...
- 피보나치 수열에서 n번째 수를 반환하는 코드를 작성한다고 해보자.
- 잠시 멈추고 이 함수 이름을 뭐라고 지을지 생각해보자.
- 보통 사람들은 이를 `fib`라 부를 것.
- 그럼 `Fib.fib(n)`으로 지어야 할까?
- 저자는 `of`나 `nth`를 제안.

### Honor the Culture

- `i`, `j`, `k` 같이 한 글자를 사용하는 것을 비판하는 글들이 많음.
- 하지만 무조건 비판은 잘못.
- 프로그래밍 언어의 문화나 환경에 따라 다름.
- C 언어에서 `i`, `j`, `k`는 주로 반복문의 증감 변수로, `s`는 문자열로 사용됨.
- 어떤 언어 커뮤니티는 camelCase를, 어떤 곳은 snake_case를 선호.

> Honor the local culture

### Consistency

- 팀원 모두가 사용하는 단어들의 의미를 동일하게 이해하고 일관되게 사용하는 것이 중요.
- 이를 위해 커뮤니케이션을 많이 해야 함.
- 예를 들어, 모두가 페어 프로그래밍을 하고 종종 스위칭.
- 혹은 프로젝트 용어집을 만들고 관리.
- 모두가 용어에 익숙해졌다면 축약어도 사용 가능.

### Renaming Is Even Harder

- 모든 것은 바뀜.
- 코드는 리팩토링 되고, 용법은 바뀌며, 의미도 조금씩 교체됨.
- 이 변화에 잘 대응하지 못한다면 이름이 오해를 사게 될 수도 있고,
- 의미 없는 이름보다 더 안 좋은 결과를 초래.
...
- 소프트웨어를 작성하도록 부탁하는 사람을 고객이라고 부르자.
- 일반적인 고객은 요구를 들고 찾아옴.
- 신입 개발자들의 흔한 실수는 그 문장 그대로 구현을 한다는 것.
- 하지만, 첫 요구 문장은 절대적 요구사항이 아님.
- 이는 고객의 진짜 요구를 탐험하기 위한 초대장.

> 주문 비용이 50$가 넘어가면 배송은 무료가 되어야 합니다.

- 위 요구사항을 처음 받았다면 잠시 생각해보자.
- 그럼 아래 질문들이 떠오름.

> 50$는 세금 포함인가?<br/>
> 50$는 배송 요금도 포함인가?<br/>
> 50$는 종이 책에만 한정인가, 아니면 이북도 포함인가?<br/>
> 어떤 종류의 배송이 대상인가?<br/>
> 국제 주문은 어떤가?<br/>
> 50$ 제한이 미래에 얼마나 자주 바뀔 수 있는가?

- 질문을 받았다면 사람들을 귀찮게 하며 에지 케이스를 찾고 이에 관해 물어라.
- 고객이 이미 고민한 것이라면 좋은 것이고, 아니라면 탐색을 시작할 차례.
- 배송을 포함한 모든 비용을 통틀어 50$라면 배송비가 무료라는 답을 들었다면,
- 만약 상품은 25$인데 고객이 새벽배송을 골라서 배송비만 30$인 경우에도 배송비를 무료로 할 것인지 질문.
- 경험 많은 개발자는 이렇게 팩트만 전달하고 결정은 고객에게 맡김.

### Requirements Are a Process

- 앞선 예제에서 개발자가 요구사항을 받고 피드백을 고객에게 전달.
- 이제 탐험이 시작된 것.
- 고객은 이런 저런 해결책을 생각해 보게 되고,
- 개발자는 더 많은 피드백을 전달하게 됨.
- 이것이 요구사항 수집.
- 프로젝트가 끝날 때까지 요구사항 수집의 연속.
...
- HR 시스템을 만든다고 가정해보자.
- 고객이 "감독관과 인사과만 고용인들의 기록을 볼 수 있게 해주세요."라고 함.
- 이것이 요구사항 문장일까?
- 비즈니스 정책과 요구사항 사이에는 미묘한 차이가 있음.
- 이 차이는 개발자에게 있어 매우 중요.
- 아래 2 문장을 비교.
- "오직 감독관과 인사과만 고용인들의 기록을 볼 수 있음"
- "오직 권한 있는 사용자만 고용인들의 기록에 접근 할 수 있음"
- 2번째 문장이 좀 더 유연한 시스템을 만드는 데 도움이 됨.
- 정책이 바뀔 경우 메타 데이터만 수정하면 됨.
...
- 최고의 문서는 동작하는 코드.
- 하지만 클라이언트가 원하는 바를 문서화 하는 일은 필요.
- 구현 프로세스를 돕는 마일스톤의 역할.

#### Requirements Documents Are Not for Clients

- Andy와 Dave는 과거에 완벽하고 정밀한 요구사항을 만들어내는 프로젝트에 참가.
- 하지만 이런 문서를 만드는 것은 2가지 이유로 실수.
- 첫 째로 고객은 처음에 자신이 진짜로 무엇을 원하는지 모름.
- 문서를 고객에게 가져가서 피드백을 받는 목적으로 서명을 요구할 수도.
- 하지만 두 번째 문제로, 고객은 문서를 읽지 않음.
- 고객은 다소 모호한 상위 수준에서 문제를 바라보는 한편,
- 개발자는 상세하고 미묘한 차이에 관심 있음.
- 요구사항 문서는 개발자를 위해 작성된 것이고,
- 고객에겐 지루하고 이해하기 어려운 것.

#### Requirements Documents Are for Planning

- 그럼에도 불구하고 문서는 작성되어야 함.
- 개발자들이 무엇을 해야 하는지 알아야 하기 때문.
- 이 문서는 사용자 스토리 형태를 갖는 게 좋아 보임.
- 이런 식으로 작성되면 요구사항을 칠판에 붙일 수 있고,
- 상태와 우선순위를 나타내며 이동할 수 있음.
...
- 퍼즐을 푸는 2가지 핵심은,
- 첫 째로 앞에 놓인 제약을 인식하는 것이고,
- 둘 째로 내가 가진 자유의 정도를 인식하는 것.
- 이 과정에서 해결책을 찾게 됨.
- 아래 그림에서, 3개의 직선만을 사용하여, 모든 점을 연결한 뒤, 시작 지점으로 돌아올 수 있겠는가?

![4개의 점 연결하기](https://learning.oreilly.com/api/v2/epubs/urn:orm:book:9780135956977/files/images/four_dots_question.png)

- 일단 선입견이 없는지 찾고,
- 이 선입견이 실제의, 바꾸기 어려운 제약인지를 판단.
- 이 때 우리가 박스 안에서 생각하는지 바깥에서 생각하는지는 중요치 않음.
- ("상자 밖에서 생각하라"라는 조언에 대한 비판)
- 진짜 문제는 박스를 찾는 것.
- 즉, 진짜 제약을 식별하는 것.
...
- 과거 경험했던 한 어려운 프로젝트가 성공했던 이유로,
- 열린 공간에 한 전문가가 앚아 있고 언제든 질문/결정/논의/데모를 할 수 있었다는 것.
- 당시에 고객과도 밀접하게 일했고, 페어 프로그래밍도 했었음.
- 코딩할 때도 질문하고 노트하고 토론하는 것을 여기서 "함께 일하는" 것으로 정의.

### Pair Programming

- 페어 프로그래밍 장점 이야기.
- 인간의 인지 용량엔 한계가 있으며,
- 동료와 함께 일했을 때 그 한계들이 극복.

### Mob Programming

- 2명을 넘어서 여러 명이 함께(타이핑은 1명) 코딩하는 것.
- 어려운 문제에 적합하다고 함.

### What Should I Do?

- 혼자 코딩하고 있다면 페어를 해보고(구체적 방식 언급),
- 이미 하고 있다면 몹 프로그래밍도 해보라고 함.
- 개발자뿐만 아니라 사용자, 테스터, 스폰서 등도 함께 해보는 것을 제안.
- 협업 시에는 인간 측면을 잘 다루어 함.
- 몇 가지 팁을 소개.
- 에고가 아닌 코드를 빌드. 누가 더 똑똑하냐의 문제가 아님.
- 작게 시작. 4-5명과 몹 프로그래밍 하거나, 몇 번만 페어 하는 것으로 시작.
- 사람이 아닌 코드를 비판. "여기 블럭을 보자"가 비판 보다 더 나음.
- 다른 사람의 관점을 듣고 이해하려 노력. 다른 것은 잘못된 것이 아님.
- 다음에 더 잘하기 위해 자주 회고하라.
