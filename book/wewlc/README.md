...
## 나를 미치게 하는 라이브러리 의존 관계

- 라이브러리 클래스를 직접 호출하는 곳은 어디든 봉합부가 될 수 있음.
- 하지만 인터페이스가 아닌 실체 클래스를 제공하면서,
- final 또는 sealed로 정의되어 있거나,
- 중요 함수가 virtual로 정의되어 있지 않으면,
- 테스트 시 가짜 객체를 사용할 방법이 없음.
- 포장 클래스로 라이브러리 사용부를 캡슐화 해야 함.
- 라이브러리 내의 클래스를 직접 호출하는 곳을 여러 곳에 분산시키면 X.
- SRP 측면에서도 그러함. 라이브러리는 언제든 교체될 수 있음.
