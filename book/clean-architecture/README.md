...
- 릴리스 번호가 없다면 재사용 컴포넌트들이 서로 호환되는지 보장 X.
- 새로운 버전이 언제 출시되고 무엇이 변했는지를 사용자들이 알아야 하기도 함.
- 컴포넌트가 릴리즈 절차를 제대로 밟는다면, 재사용을 효율적으로 할 수 있음.
...
- 코드가 변경되어야 할 때, 여러 컴포넌트를 함께 수정하기 보다는, 하나의 컴포넌트만 수정할 수 있는 것이 나음.
- 릴리즈, 재검증, 배포 관련된 일의 범위를 제한할 수 있기 때문.
- CCP는 이를 돕는 원칙.
- 일반적으로 유지보수성이 재사용성보다 훨씬 중요.
...
- 재사용하는 컴포넌트가 바뀐다면, 클라이언트 측도 함께 바꿔야 할 가능성이 높음.
- 문제는 실제로는 쓰이지 않는 부분이 바뀌었다고 하더라도, 컴파일이나 테스트 등의 검증 및 재배포를 해야 할 수도 있음.
- 강하게 결합되지 않은 클래스들을 동일한 컴포넌트에 위치시키지 않아야 함.
- 불필요한 변경에 우리의 노력을 허비할 수도.
...
- 시스템 아키텍처는 시스템 동작 여부와는 거의 관련 없음.
- 형편 없는 아키텍처의 시스템도 그런대로 잘 동작한다고.
- 오히려, 시스템의 생명주기를 지원하는 것.
- 그러니까, 시스템을 쉽게 이해하고, 쉽게 개발하며, 쉽게 유지보수하고, 쉽게 배포하게 돕는 것.
- 아키텍처가 운영에 미치는 영향은 상대적으로 약함. 단순히 하드웨어를 더 투입해서 해결하는 경우들이 많음.

그리고 좋은 아키텍처를(쉬운 이해/개발/유지보수/배포) 위해 아래와 같이 해야 함을 강조.

> 이러한 일을 용이하게 만들기 위해서는 가능한 한 많은 선택지를, 가능한 한 오래 남겨두는 전략을 따라야 한다.

개인적으로는 굳이 이렇게 지면을 할애한 이유가 궁금. 앞으로 설명할 내용들이 이 정의의 범주 안에 들기 때문인가? 그렇다면 좀 더 간결하게, 필요한 내용만 작성하는 게 좋지 않았을까. 뒤를 읽다 보면 알게 되리라 기대.
