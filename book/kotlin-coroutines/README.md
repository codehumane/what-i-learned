...
- 콜백, 스레드, 리액티브 등의 다른 비동기 방식보다 많은 것을 제공.
...
- 백엔드에서는 스레드를 코루틴으로 바꾸는 대부분의 환경에서 단지 suspend 제어자를 추가하는 것으로 충분.
- 동시성을 쉽게 구현하고, 동시성을 테스트할 수 있으며, 코루틴을 취소할 수도 있고, 그 외 기능도 활용 가능.
- 그리고 스레드에 비해 가벼움.
