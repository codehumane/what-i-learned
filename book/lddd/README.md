...
- 둘 다 물리적 경계.
- 단일 팀이 소유.
- 모델 충돌이 일어나는 것을, 하나의 MS나 BS에 두는 것도 어려움.
- 하지만 일관성 단위가 크다면?
- 모든 MS는 BC로 볼 수 있지만,
- 모든 BC가 MS인 것은 아님.

![](./lddd_bc_ms_module_and_size.jpg)

다음으로 애그리게이트.

- BC는 가능한 넓은 유효한 경계를 설정하지만,
- 애그리게이트는 가능한 한 좁게 설정.
- 애그리게이트는 비즈니스 규칙, 불변성, 복잡성을 감싸는 개별적 비즈니스 기능 단위.
- 애그리게이트를 MS 단위로 고려하면 이는 얕은 서비스이며 글로벌 복잡성 커짐.
- 서비스는 시스템 전체의 복잡성까지 고려해야 함.

마지막으로 하위 도메인.

- 하위 도메인의 경계를 MS의 것과 일치시키는 것이 좀 더 균형잡힌 휴리스틱이라 함.
- 하위 도메인은 세분화된 비즈니스 역량과 관련.
- 기술적 관점에서는 응집된 유스케이스의 집합을 대표함.

## CH15. 이벤트 주도 아키텍처

- EDA. 참고로, 이벤트 소싱과는 다름.
- 부주의한 EDA 적용은 분산된 커다란 진흙 덩어리.
- 이번 장에서는 EDA와 DDD의 상호작용,
- EDA의 필수 요소와 EDA 실패의 일반적인 원인,
- DDD를 통해 효과적인 EDA 설계를 다룬다고 함.

### 이벤트 주도 아키텍처

- EDA: 시스템 컴포넌트가 이벤트 메시지를 교환하면서 비동기로 서로 커뮤니케이션.
- 이벤트 주도 실행 흐름의 일반적 예는 앞서 소개했던 사가 패턴.
- 이벤트 주도 설계 != 이벤트 소싱
- 이벤트 소싱은 상태의 변화를 일련의 이벤트로 캡처하는 방법.
- EDA는 서비스 간 통신을 의미, 이벤트 소싱은 서비스 내부에서 발생.
