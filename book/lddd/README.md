...
- 이벤트는 메시지이지만,
- 메시지가 반드시 이벤트는 아님.
- 메시지에는 2가지 유형, 이벤트와 커맨드가 있음.
- 이벤트는 이미 일어난 일.
- 커맨드는 어떤 일을 하라는 지시.
- 둘 다 비동기로 통신 가능.
- 하지만 커맨드는 거부될 수 있음.
- 이벤트는 수신자가 취소 불가.
- 유일한 방법은 사가 패턴에서의 보상 조치.
...
- `PaycheckGenerated`, `CampaignPublished` 등.
- 장황하지 않고 식별자만 포함. 상세 정보는 이벤트에 포함된 링크로.
- 재난 문자도 이벤트 알림 메시지의 한 예.

다음으로, 이벤트를 통한 상태 전송.

- 구독자에게 제공자의 내부 상태 변경을 알려줌.
- 상태 변경을 나타내는 모든 데이터가 메시지에 포함됨.
- 이 변경은 스냅샷으로, 또는 수정된 부분만 담아 나타낼 수도.
- 이벤트를 상태 전송은 개념적으로 비동기 데이터 복제 메커니즘.
- 사용자는 이벤트 스트림을 통해 엔티티 상태의 로컬 캐시를 보유하고 이용하기도.
- 데이터 제공자가 가용하지 않은 경우에도 사용자는 필요한 일들을 할 수 있으므로 내결함성 향상.
- 성능의 이점도 있음.

마지막, 도메인 이벤트.

- 이벤트 알림, 이벤트를 통한 상태 전송과 유사하나 다름.
- 일단, 제공하는 데이터 성격이 다름.
- 이벤트 알림과 달리 도메인 이벤트에는 이벤트 설명하는 정보를 충분히 포함.
- 한편, 이벤트를 통한 상태 전송처럼 로컬 캐시를 할 정도의 충분한 데이터 제공은 아님.
- 도메인 이벤트에는 주요 비즈니스 변경을 알리고 이를 설명하는 정도의 데이터만 가짐.
- 다음으로, 모델링 의도도 다름. 
- 이벤트 알림은 다른 컴포넌트와의 연동을 돕기 위해 설계.
- 이벤트를 통한 상태 전송은 데이터 애그리게이트의 상태를 설명하는 것.
- 도메인 이벤트는 비즈니스 도메인의 생애 주기 동안 발생한 비즈니스 이벤트를 설명.
