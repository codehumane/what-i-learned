...
2PC의 평판에 대해서는 서로 엇갈림. 안전함을 보장해주는 중요한 도구라는 평가가 있는 반면, 운영상 문제를 일으키고 성능을 떨어뜨리며 지나친 약속을 한다(?)는 비판도 있음. 많은 클라우드 서비스들이 이런 이유로 분산 트랜잭션을 구현하지 않음. 분산 트랜잭션은 무거운 성능 페널티를 가져옴. MySQL에서 분산 트랜잭션은 단일 노드의 트랜잭션에 비해 10배 이상 느리다고 보고됨. 충돌 복구에 필수인 추가적인 디스크 포싱(fsync), 그리고 추가적인 네트워크 라운드 트립 때문이라고 함.

하지만, 분산 트랜잭션을 통해 얻을 수 있는 교훈이 있고, 따라서 좀 더 자세히 살펴보려 함. 먼저, "분산 트랜잭션(distributed transactions)"가 뭔지부터 명확히 해야함. 보통 아래 2가지 개념이 혼동됨.

1. Database-internal distributed transactions
2. Heterogeneous distributed transactions

첫 번째는 같은 데이터베이스의 노드간 트랜잭션(VoltDB나 MySQL Cluster's NDB 저장소 엔진). 두 번째는 서로 다른 기술 간의 트랜잭션. 2개의 서로 다른 벤더사의 데이터베이스나, 메시지 브로커 같은 데이터베이스가 아닌 시스템까지. database-internal 트랜잭션은 다른 시스템과의 호환이 필요 없으므로, 어느 프로토콜이나 쓸 수 있고 특정 기술에 특화된 최적화도 가능. 이런 이유로 database-internal 트랜잭션은 잘 동작함. 반면, heterogeneous 방식은 도전적.
...
우리는 *in doubt* 상태에 놓인 트랜잭션에 왜 그렇게 많은 신경을 쓰는걸까? 시스템의 나머지 부분에서는 이를 무시하면 되는 것 아닐까? 이 트랜잭션은 결국에는 정리가 될텐데?

- 문제는 *locking*.
- 데이터베이스는 트랜잭션이 *commit* 또는 *abort* 될 때까지 락을 해제할 수 없음.
- 코디네이터에 장애가 생기고 복구하는 데 20분이 걸린다면 락은 20분 동안 지속될 것.
- 또, 어떤 이유로 코디네이터의 로그가 완전히 사라진다면 락은 영원히 지속. (관리자가 해결해 주지 않는다면)

개인적인 생각이지만, 이런 부분 때문에 XA는 더더욱 위험. 인스턴스는 언제든 버려질 지도 모르는 일(immutable deploy도 한 번 생각해 보라). 또, 같은 DB라도 분리가 되어 있거나, 심지어 아예 서로 다른 기술이 사용된다면, 트랜잭션으로 강하게 묶여야 할 가능성도 매우 낮아 보임. 정말 필요한 일인지부터 고민해봐야 하지 않을까. 혹은 경계가 잘못 나뉘어 진 것.

#### Recovering from coordinator failuer

- 이론상으로는 코디네이터에 장애가 생기면,
- 재시작을 했을 때 로그로부터 상태를 복구하고 in-doubt 상태에 있는 트랜잭션을 해결해야 함.
- 하지만, 실제로는 고아<sup>orphaned</sup> in-doubt 트랜잭션이 일어남.
- 로그가 사라졌거나 소프트웨어 버그 등의 이유로 트랜잭션의 결과를 코디네이터가 결정할 수 없기 때문.
- 결국 관리자가 수동으로 커밋 또는 롤백을 시켜줘야 함.
- 일단 in-doubt 상태의 트랜잭션을 찾아야 하고,
- 각 참여자들이 커밋을 했는지 아니면 종료를 했는지 알아내야 하며,
- 일관된 결과를 각 참여자들에게 반영시켜 줘야 함.
- XA 구현체들이 *heuristic decision* 이라는 피난용 도구를 제공해 주긴 하지만,
- (이는 참여자들이 일방적으로, 코디네이터의 관여 없이, 커밋 또는 종료를 결정하게 하는 것)
- 하지만 이는 원자성을 깨뜨리는 잠재적 원인이 됨. 2PC의 약속을 어길 수 있기 때문.
- 따라서, 심각한 상황에서만 예외적으로 사용되어야 함.

#### Limitations of distributed transactions

분산 트랜잭션의 여러 가지 문제점에 대해 이야기. 당연한 것들.

- 코디네이터가 단일 장비에서 동작한다면? (놀랍게도 꽤 많은 경우에 해당한다고 함)
- 애플리케이션 인스턴스가 쉽게 제거되고 추가되는 데 장애물이 됨. (심지어 쉽게 죽이지도 못하지 않나. 기다려야 할 듯)
- XA가 여러 시스템 간에 호환되어야 하므로 최소한의 공통 분모만 가질 수 있음. 예컨대, 여러 시스템을 아울러 충돌을 감지할 수 없기에, 서로 다른 시스템 간의 데드락을 감지할 수 없고, SSI를 할 수 없는 등의 문제를 가짐.
- 시스템 하나라도 문제가 생기면 장애가 전파되는 amplifying failure 특성을 가짐. 장애 내성에 약함.

### Fault-Tolerant Consensus

약식으로 컨센서스는 몇 개의 노드로부터 무언가에 대한 동의를 이끌어 내는 것을 가리킴. 컨센서스 알고리즘은 "which one of these mutually incompatible operations should be the winner"를 결정하는 데 사용. 일반적으로 아래처럼 공식화 됨.

1. 1개 이상의 노드가 값을 제안(*propose*)
2. 컨센서스 알고리즘은 이 중에 하나를 결정(*decide*)

그리고 몇 가지 속성을 만족시켜야 함.

1. Uniform agreement: 어떠한 두 노드도 서로 다른 결정을 하지 않음.
2. Integrity: 어떠한 노드도 두 번 결정하지 않음(번복하지 않는다는 말).
3. Validity: 만약 한 노드가 v 값을 결정했다면, v는 어떤 노드에 의해 제안된 것.
4. Termination: 장애가 나지 않은 모든 노드는 결국에는 값을 결정하게 됨(뒤에서 다루지만 항상 가능한 것은 아님. 정족수라는 제약).

uniform agreement와 integrity가 핵심 아이디어. 모두가 같은 결과를 결정하고, 한 번 결정되면 바뀌지 않음. validity 속성은 자명한해(trivial solution, 의미를 찾아보니 y = 0 같은 쓸모없는 해 등을 가리키는 말)를 걸러내기 위한 것. 예컨대, 무슨 값이 제안되든 항상 null 값을 결정하는 알고리즘을 배제(이는 uniform agreement와 integrity는 만족시킴).

장애 내성을 생각하지 않는다면, 처음 3개 속성은 만족시키기가 매우 쉬움. 한 노드를 결정자(책에서는 "dictator"라고 써 놓음)라고 하드코딩 해 두고, 그 노드가 모든 결정을 내리게 하면 됨. 하지만 그 노드에 문제가 생긴다면? 여러 가지 문제가 예상됨. 이를 극복하기 위한 속성이 termination. 아무것도 하지 않는 대신, 계속 앞으로 나아가게 함. 일부 노드가 죽더라도, 다른 노드들은 여전히 결정을 내리는 것. 참고로, Termination은 liveness 속성. 반면, 나머지 3개는 safety 속성.

컨센서스 모델에서는 노드가 죽어서(crashes) 사라진 뒤 다시는 돌아오지 않음을 가정함. 노드가 복구되길 기다리는 알고리즘은 termination 속성을 만족시킬 수 없음. 2PC가 대표적인 예. 물론, 모든 노드가 죽는다면 결정을 내릴 수 없음. 컨센서스 알고리즘은 노드의 과반수 이상이 정상적으로 동작하고 있어야 termination을 보장할 수 있음. 정족수. 따라서, 대규모 정전은 요청의 처리를 멈추게 할 수는 있지만, 컨센서스 시스템이 잘못된 결정을 내리게는 할 수 없음.
...
지금까지 살펴본 모든 컨센서스 프로토콜은 리더가 고유함에 대한 약한 보장을 제공.

- 리더가 고유함을 보장하지는 않음. 대신, 약한 보장을 제공.
- *epoch number*를 정의하고, 각 epoch 내에서 리더는 고유함을 보장하는 식.

투표는 다음과 같은 방식으로 진행.

- 현재 리더가 죽었다고 판단될 때 마다, 새로운 리더 선출을 위한 투표가 시작.
- 투표에는 증가된 epoch number가 주어지며, 이 숫자는 totally ordered이고 일정하게 증가.
- 서로 다른 epoch를 가진 서로 다른 리더가 충돌한다면 높은 값을 가진 리더가 우선.
- 리더가 선출이 되면 일을 시작하기에 앞서 다른 노드에게 충돌이 없는지를 확인.
- 이 확인은 과반수 이상의 노드(quorum)로부터 승인 응답을 받는 것으로 이뤄짐.
- 결국 2 번의 투표가 이뤄지는 것. 일단 리더가 선출되고, 리더의 제안에 대한 또 다른 투표를 진행.

좀 더 자세한 내용은 [여기](https://www.sciencedirect.com/topics/computer-science/consensus-algorithm)를 살펴보는 게 도움이 됨. 아래 문단부터 살피면 됨.
...
더불어, 이 투표 과정을 2PC와 비교하고 있음.

- 2PC에서는 코디네이터가 선출되지 않음.
- 컨센서스에서는 과반 수 이상의 투표를 필요로하지만,
- 2PC에서는 모든 참여자들로부터 "yes"를 받아야 함.
- 게다가, 컨센서스는 리더가 선출 된 뒤 일관성을 검사하여 안전성<sup>safety</sup> 속성을 항상 만족시킴.
- 이 차이가 컨센서스의 정확성<sup>correctness</sup>과 장애 내성<sup>fault tolerance</sup>을 만들어 냄.

#### Limitations of consensus

safety 속성도 만족시키고, 동시에 fault-tolerant. total order broadcast를 제공하는 등 여러 이점을 가짐. 하지만 분명한 비용도 뒤따름.

- 제안에 대한 투표가 동기적으로 이뤄짐. not asynchronous.
- 과반수 이상의 노드가 필요함.
- 클러스터에 동적 노드 추가나 삭제가 어려움. 대부분의 컨센서스 알고리즘은 미리 정해진 노드들이 투표에 참여한다고 가정.
...
- 그리고 분산 락은 일반적으로 *lease*([Process Pause](/book/ddia/Distributed-Data.md#process-pauses)에서 다뤘던)를 이용해서 구현됨.

*Total ordering of operations*

- lease나 lock을 사용할 때, process pause 등에 의한 클라이언트의 충돌을 막기 위해 [fencing token](/book/ddia/Distributed-Data.md#fencing-tokens)을 사용.
...
ZooKeeper/Chubby 모델의 사용 예가 몇 가지 있음. 대표적으로, 여러 서비스/프로세스 인스턴스가 있고, 이 중에 리더를 뽑아야 하는 경우(페일오버 포함). 싱글 리더 데이터베이스나 잡 스케쥴러, 그리고 이와 유사한 상태가 있는<sup>stateful</sup> 시스템들이 구체적 예. 또한, 파티셔닝 된 리소스에서, 어떤 노드로 어떤 파티션을 할당할지 결정하는 데에도 사용(리밸런싱 포함). 이 때는 ZooKeeper의 원자적 연산, ephemeral 노드, 알림 등을 활용.

너무 많은 노드들 간에 과반수 투표를 하는 것은 비효율적. ZooKeeper는 보통 3개나 5개의 고정된 수의 노드만을 운영. 그리고 이 노드 간에 투표를 진행. 그래서 애플리케이션의 수가 늘어났을 때, ZooKeeper에게 투표를 "아웃소싱"하는 것. 그 외에도 여러 코디네이팅 작업들을 아웃소싱.
...
ZooKeeper와 같은 도구들은 *membership services*에도 활용 가능. 항공 교통 제어 등 높은 신뢰성을 중요하게 생각하는 분야에서 활용. membership service는 클러스터의 어떤 멤버들이 활성화 되어 있고 살아 있는지를 결정함. 노드의 장애 탐지와 더불어 컨센서스를 같이 사용하면 split brain을 막을 수 있기 때문에 좀 더 유용함.
