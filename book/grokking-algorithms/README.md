...
- KNN을 이번엔 넷플릭스 사용자들의 비슷한 취향을 분류하는데 사용.
- 그래프를 그리고 사용자들의 위치를 정하고,
- 특정 사용자와 가장 가까운 위치에 있는 사용자들을 찾는 것.
- 세부적인 내용은 아직까지 없음.
...
- 과일이 이번엔 3개라고 해보자.
- 위 표를 그래프로 나타내면, 누가 비슷한지 시각적으로 판단 가능.
- 또는 피타고라스 정리로 구하면 됨.

$$\sqrt{(x_{1} - x_{2})^2 + (y_{1} - y_{2})^2}$$

- 이를 기반으로 아래 사용자들의 선호도로 거리 구하기 가능.
...
- 추천에서 좀 더 나아가서,
- 사용자가 주어진 영화를 어떻게 평가할지 예측해 볼 수도 있음.
- 일단, 거리(위에서 얘기했던 거리)가 가까운 사용자 k명을 고르기.
- 이 k명의 영화에 대한 평가를 평균 내어 예상치로 사용.
- 이런 방식을 가리켜 회귀<sup>regression</sup>라 부름.
- KNN으로 할 수 있는 건 그래서 2가지.
- 분류<sup>classification</sup>와 회귀<sup>regression</sup>
- 분류는 그룹으로 범주화하는 것이고, 회귀는 반응을 예측하는 것.

다음으로, 빵을 얼마나 구워야 하는지 예측하는 예시 소개.

|   | weather | weekend or holiday | game | loaves |
| - | ------- | ------------------ | ---- | ------ |
| A | 5 | 1 | 0 | 300 |
| B | 3 | 1 | 1 | 225 |
| C | 1 | 1 | 0 | 75  |
...
- 위와 같이 날씨 상황, 주말/휴일 여부, 게임 유무에 따른 빵의 과거 판매량이 있음.
- 오늘은 주말이고 날씨는 괜찮으므로 `(4,1,0)`.
- 이 데이터와 가까운 것들은 A, B, D, E.
- 그러니 대략 218 정도로 예측할 수 있음.

### Picking good features

- KNN에서 올바른 피처를 고르는 것은 매우 중요.
- 추천해주려는 대상과 직접적인 연관성을 가져야 하고,
- 편향이 없어야 함(코미디 영화만을 주고 고르라고 하면 액션 영화에 대한 것을 알 수 X).

## Introduction to machine learning

- KNN은 매우 유용하며 머신 러닝의 도입부.
- OCR, 스팸 필터, 주식 시장 예측 사례들을 추가로 간략하게 소개.
- 내용은 간단해서 기록 생략.
