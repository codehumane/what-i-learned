...
- 웹 서버는 TCP 커넥션을 요청 받으면 이를 수락하고, 어떤 클라이언트인지 확인을 위해 IP를 추출함.
- 웹 서버는 일반적으로 리버스 DNS를 사용해서 클라이언트의 IP 주소를 호스트 명으로 변환.
- 하지만 hostname lookup은 많은 시간이 걸림. 대용량 웹 서버는 호스트 명 분석을 꺼두거나 특정 컨텐츠에 대해서만 허용.
...
- 서버마다 여러 커넥션을 처리하기 위한 아키텍처를 선택함. 아래 그림 참고.
...
- docroot
  - 웹 서버는 파일 시스템의 특별한 폴더를 웹 컨텐츠를 위해 예약. URI를 이 문서 루트에 붙여서 찾음.
...
- 동적 컨텐츠 리소스 매핑
- 서버사이드 인클루드<sup>Server-Side Includes, SSI</sup>
