...
- 가장 많이 쓰이고 믿음직스러운 최종 일관성 패턴.
- 이벤트나 커맨드를 이벤트 형태로 비동기 발행.
- 분산 트랜잭션에 참여한 서비스들이 이를 받아 적절히 대응.
- 보통 병렬로 진행되므로 응답성이 나쁘지 않음.
- 서비스들도 디커플링.
- 하지만 에러 처리가 복잡.
- 몇 번의 재시도와 DLQ 활용 등 복잡성 증대.
...
- 서비스가 자신이 소유하지 않은 데이터를 읽는 방법으로,
- 4가지 패턴을 소개.
- 설명을 위해 위시리스트와 카탈로그 서비스 다시 활용.
- 고객은 자신의 위시리스트를,
- Product에 있는 item_desc와 함께 보길 바란다는 시나리오로 출발.

| 서비스 | 테이블 | 컬럼 |
| ----- | ---- | --- |
| 위시리스트 | Whishlist | cust_id, item_id, date_added |
| 카탈로그 | Product | item_id, item_desc, dimensions |

### 10.1 서비스 간 통신 패턴

- 가장 일반적 패턴.
- 데이터를 소유한 서비스에게 그냥 데이터를 요청하는 것.
- 간단하긴 하나 단점이 많음.
- 다름 아닌 성능(네트워크 레이턴시, 보안 레이턴시, 데이터 레이턴시).
- 더욱 큰 단점은 서비스 커플링.
- 위시리스트 -> 카탈로그 서비스.
- 카탈로그 실패는 위시리스트 서비스의 가용성 문제로 이어질 수도.
- 사용량이 늘어날 때도 함께 확장해줘야 하는 결합도 문제도.

### 10.2 컬럼 스키마 복제 패턴

- 데이터를 필요로 하는 곳에 복제해서 사용하는 것.
- 여기선 item_desc 값이 Product 테이블 외에 Whishlist에도 존재.
- 이 방식의 문제점은 데이터 동기화와 일관성.
- 변경이 일어날 때마다 이를 알리고 동기화 해야 하고,
- 이런 작업이 누락되거나 의도치 않은 쓰기 등에 의해 데이터 비일관성 문제도.
- 하지만 성능, 내고장성, 확장성이 개선됨.
- 저자는 이 시나리오에서는 사용하지 말 것을 권장.
- 대신, 데이터가 워낙 대용량이라 높은 응답성이나 내고장성이 절실하며 다른 패턴 적용이 어려울 때 사용.
- 예를 들면, 데이터 집계나 리포팅.

### 10.3 복제 캐싱 패턴

- 컬럼 대신 메모리 캐시에 데이터를 복제하는 것.
- 그런데 복제 캐싱은 다른 캐싱 모델과는 다름.
- 복제 캐싱은 데이터가 저장된 모든 서비스가,
- 언제나 정확히 동일한 데이터를 참조하도록 계속 동기화.
- 분산 캐싱과 대조되는 용어인 복제 캐싱임에 유의.
- 분산 캐싱은 내고장성, 레이턴시를 이유로 여기서는 추천 X.
- 한편 복제 캐싱의 단점들은 아래와 같음.
- 먼저, 서비스가 cache data와 startup timing 관련 의존성을 갖게 됨.
- 처음 서비스를 시작할 때 카탈로그 서비스가 가용해야 하는 것.
- 다음으로, 데이터의 양이 너무 크면 실용성이 떨어짐.
- 세 번째로, 변경 빈도가 높을수록 데이터 동기화 어려움.
- 마지막으로, 구성 및 설정 관리가 어렵다는 문제도 있음.
- 보통 서로를 브로드캐스팅이나 룩업을 통해 인지하는데,
- 범위가 커지면 소켓 레벨의 핸드세이크 시간이 오래 걸리며,
- 클라우드 같이 주소가 동적인 환경에서 설정과 제어가 어려움.

### 10.4 데이터 도메인 패턴

- 앞에서 다룬 데이터 도메인 기법과 같음.
- Whishilist, Product를 하나의 공유 디비/스키마에 두는 것.
- 둘 중 누구도 독점하지 않고 공유.
- 일반적으로는 권장하지 않음.
- 경계 컨텍스트가 넓어지고, 오너십 관리가 모호하며, 보안적인 문제 등 때문.
- 책에서는 서비스 간 결합도가 낮아진다 하지만, 실제로는 테이블 변경이 일어날 땐 서로 간의 커플링이기도 함.
- 다만, 서비스 간 가용성 디펜던시, 응답성, 처리량, 확장성, 데이터 일관성에 유리.

## CH11. 분산 워크플로 관리

- 분산 환경에서 상호작용 모델을 고민할 때,
- 3가지 요소인 통신, 일관성, 조정이 있었음.
- 통신은 동기냐 비동기냐.
- 일관성은 원자적이냐 결과적이냐.
- 조정은 코레오그래피냐 오케스트레이션이냐.
- 이번 장은 조정<sup>coordination</sup>에 관한 것.

### 11.1 오케스트레이션 통신 스타일

- 오케스트레이터 컴포넌트를 따로 두고,
- 워크플로 상태, 선택적 로직, 에러 처리, 알림 등을 관장.
- 일반적으로 오케스트레이터에는 워크플로 중재 외에 다른 일을 맡기진 않음.
- 참고로, ESB와 같은 전역 오케스트레이터 아님. 디커플링이 목표이기 때문.
- 대신 워크플로우 당 오케스트레이터를 둠.
- 주문을 예로 들며, 정상 케이스와, 결제나 실제 주문 이행 시 문제가 생기는 경우를 간략하게 소개.

### 11.2 코레오그래피 통신 스타일

- 오케스트레이터가 없고, 각 서비스들끼리 협업.
- 정상 경로와 실패 케이스에 대한 간단한 소개 나옴.
- 마지막엔, 시멘틱 복잡성과 구현 복잡성에 대한 서로 다른 태도 이야기.
- 시멘틱 복잡성은 어쩔 수 없음. 하지만 불필요한 구현 복잡성을 더하지는 않아야.
- 구현이 시멘틱을 더 나쁘게 만들 수는 있어도 더 좋게는 만들 수 없음.
- 여기서는 전형적인 레이어드 아키텍처를 구현 복잡성 올리는 사례로 언급.
- 이 대신 모듈리스 모놀리스를 권장.

#### 11.2.1 워크플로 상태 관리

- 오케스트레이션에서는 오케스트레이터가 상태의 소유자.
- 상태란, 실행된 단계는 어디이고 얼마나 남아 있으며 실행 순서나 에러 조건 등을 가리킴.
- 그런데 코레오그래피에서는 어떻게 할까?
- 이를 위한 3가지 패턴 소개.
- 첫째는 front controller.
- 가장 먼저 호출된 서비스가 상태 관리를 담당하는 것.
- 위 사례에서는 주문 서비스가 이를 담당.
- 그러나 이렇게 되면 다른 서비스들이 상태 업데이트를 위한 추가 통신이 필요.
- 둘째는 stateless choreography.
- 여기서는 상태를 저장하지 않고, 각 서비스를 쿼리해서 실시간 스냅샷을 구축.
- 고객이 주문 상태를 조회하면 각 서비스가 호출되어 주문 상태를 실시간으로 결정.
- 마지막으로 stamp coupling.
- 서비스 간 전송되는 메시지 계약에 워크플로 상태를 끼워 넣는 것.
- 각 서비스는 전체 상태 중 자신에게 해당되는 부분을 업데이트 후 다음 서비스에 전달. 

### 11.3 오케스트레이션과 코레오그래피의 트레이드오프

#### 11.3.1 상태 오너 및 커플링

- 코레오그래피: 응답성과 확장성이 필요하고 에러 시나리오가 복잡하지 않거나 빈도가 낮은 곳.
- 오케스트레이션: 경계 조건과 에러 조건이 복잡한 워크플로우에 적합.
- 이 외에 여러 장단점 얘기하고 있으나 너무 일반화되고 세부사항 고려가 부족하여 참고용일 뿐. 기록은 생략.
- The devil is in the detail.

## CH12. 트랜잭셔널 사가

- 가볍게 훑고 지나갈 줄 알았으나 40페이지를 넘게 차지하고 있어 내용이 궁금.
- 아래는 사가에 관한 역사 이야기.

> 사가는 마이크로서비스 이전에 등장한 개념으로, 원래 초기 분산 아키텍처에서 데이터베이스 락의 범위를 제한하는 것에 관한 주제였습니다.

### 12.1 트랜잭셔널 사가 패턴

- 책 앞에서 3가지 동적 커플링 얘기했었음.
- 통신, 일관성, 조정.
- 트랜잭셔널 사가 패턴 별 이 커플링을 표로 정리.

| 패턴 | 통신  | 일관성     | 조정        |
| --- | ---- | -------- | ---------- |
| SAO | 동기  | 원자성     | 오케스트레이션 |
| SAC | 동기  | 원자성     | 코레오그래피   |
| SEO | 동기  | 최종 일관성 | 오케스트레이션 |
| SEC | 동기  | 최종 일관성 | 코레오그래피   |
| AAO | 비동기 | 원자성     | 오케스트레이션 |
| AAC | 비동기 | 원자성     | 코레오그래피  |
| AEO | 비동기 | 최종 일관성 | 오케스트레이션 |
| AEC | 비동기 | 최종 일관성 | 코레오그래피  |

#### 12.1.1 SAO

- 전통적인 사가 패턴으로 알려졌으며, orchestrated saga 라고도 불림.
- 참여한 서비스를 하나씩 호출하고, 하나라도 실패하면 이전에 성공한 요청들에 대해 보상 트랜잭션.

#### 12.1.2 SAC

- SAO에서 조정을 오케스트레이션에서 코레오그래피로 바꾼 것.
- 오케스트레이터 없고 서비스들끼리 직접 협력.
- 앞서 말한 front controller 사용.
- 사실 코레오그래피를 선택하면 대부분 비동기라,
- 이 패턴은 흔치 않음.
...
- 다음 서비스로 요청을 전달.
- 전체 트랜잭션은 없음.
- [Chain of Responsibility](https://refactoring.guru/design-patterns/chain-of-responsibility), [Pipes and Filters](https://www.enterpriseintegrationpatterns.com/PipesAndFilters.html) 스타일 활용.

#### 12.1.5 AAO

- 이는 주로 잘못된 해결책.
- SAO의 성능을 높일 수 있다는 잘못된 믿음에서 기인한다고.
...
- 전통적 SAO에서,
- 제약을 완화하고 구현을 쉽게 만든 것.
- 중재자가 조정에 유용하게 쓰임.
- 대용량 데이터가 오가는, 복잡한 워크플로우에서 고려해 볼 만.

#### 12.1.8 AEC

- AEO는 복잡한 곳에,
- AEC는 단순하면서 응답성/확장성이 중요한 곳에.

#### 정리

지금까지 나온 내용을 표로 정리하면 아래와 같음.
...
- 유한 상태 기계 개념을 활용해,
- 트랜잭셔널 사가 상태를 파악하고,
- 재시도나 자동/수동 조치를 통해 오류를 해결하는 방법.
- SEO를 활용한 티켓 완료 프로세스를 예시로 설명.

#### 12.2.1 사가 상태 기계

먼저, 사가 상태 기계 간단 요약.

- 상태 기계는 분산 아키텍처에서의 모든 가능한 경로를 기술하는 패턴.
- 트랜잭셔널 사가를 개시하는 beginning state로 시작.
- 상태가 변경될 때마다 수행할 액션과 transition state가 있음.

그리고, 고장 티켓은 아래 흐름대로 처리한다고 해보자.

1. 고객이 고장 티켓을 시스템에 새로 등록.
2. 출동 가능한 기사에게 티켓을 배정.
3. 기사의 모바일 기기로 티켓 전송.
4. 기사는 티켓을 받고 고객 방문 후 수리 진행.
5. 수리 완료 후 기사는 티켓을 완료 처리.
6. 고객에게 설문 발송.

여기에 필요한 상태는 8개이며 설명은 별 것 없어서 생략. 오른쪽 괄호는 전이가 일어날 수 있는 상태 번호 목록.

| 변경 전 상태 | 변경 후 상태 | 액션 | 
| ---------- | --------- | --- |
| START | CREATED | 신규 티켓 생성 |
| CREATED | ASSIGNED | 기사에게 티켓 배정 |
| ASSIGNED | ACCEPTED | 기사가 수리 시작 |
| ASSIGNED | REASSIGN | 기사의 티켓 거부 |
| REASSIGN | ASSIGNED | 다른 기사에게 티켓 재배정 |
...
- 서로 어떻게 통신할지에 대한 정의.
- gRPS, SOAP, REST, XMLRPC 등은 계약 포맷.
- 책에서는 하드 파트 계약이라고 부르며 아래와 같이 정의.

> 어떤 정보나 디펜던시를 전달하기 위해 아키텍처의 각 파트에서 사용되는 포맷.

### 13.1 엄격한 계약 vs. 느슨한 계약

- 엄격하고 느슨한 정도도 스펙트럼에 다양하게 분포.
- RMI, gRPC는 가장 엄격한 계약의 사례.
- 이름, 타입, 순서, 그 밖의 모든 세부 사항을 준수하며 모호한 부분을 남겨 두지 않음.
- JSON은 보통 느슨한 계약이지만, [스키마를 추가하면 엄격해짐](https://json-schema.org/).
- GraphQL은 값비싼 조정 없이도 다양한 종류의 서비스에서 읽기 전용 집계 데이터 제공하는 데 사용.
- 이 때 처음부터 많은 정보를 담으면 나중에 변경에 취약해짐.

#### 13.1.1 엄격한 계약과 느슨한 계약의 트레이드오프

엄격한 계약의 트레이드오프.

| 장점 | 단점 |
| --- | --- |
| 계약으로 스키마 검증하니 정확한 준수 가능 | 커플링 심함 |
| 빌드 시 쉽게 검증 가능 | 버저닝 부담 |
| 문서화 쉬움 | |
...
| --- | --- |
| 디커플링 | 오타 등으로 결함 가능 |
| 계약을 발전 시켜나가는 데 유리 | 컨슈머 주도 계약 등 안정성을 위한 여러 조치가 필요 |

#### 13.1.2 마이크로서비스에서의 계약
...
- [Stamnp Coupling 설명은 여기 참고](https://wiki.c2.com/?StampCoupling).
- 이렇게 되면 계약에 불필요한 세부 사항이 너무 많이 노출될 수 있고,
- 지나치게 많은 대역폭을 사용하게 될 수도 있음.
- 이 방식은 워크플로 관리에서도 언급했음.
- 코레오그래피를 사용할 수 밖에 없고,
- 그런데 워크플로우가 너무 복잡하다면,
- 스탬프 커플링의 단점을 감수하고도 고려해 볼 만.

## CH14. 분석 데이터 관리

별 내용 없을 줄 알았으나 생각보단 재밌게 읽어서 기록.

### 14.1 예전 접근 방법

- 데이터 웨어하우스에서 데이터 레이크로.
- 변환 후 적재하던 모델이, 적재 후 변환하는 모델로 바뀐 것.
- 쓰지도 않을 불필요 데이터 변환에 밚은 비용을 들이기보다,
- 있는 그대로 두고 필요한 곳에서 직접 변환/가공하기.
- 변환 문제는 어느 정도 해결했지만, 여전히 많은 한계점이 있음.
- 컨슈머 입장에서 이질적인 여러 데이터를 어떻게 연관시켜야 할지 부담.
- 변환/가공이 줄어 민감 정보들이 노출될 위험.
- 도메인 분할이 선호되는 요즘, 데이터 레이크는 여전히 기술 분할.

### 14.2 데이터 메시

- 사이드카, 서비스 메시와 같은 방식.
- 비즈니스 도메인에 따라 아키텍처와 데이터 오너십을 구성하여 피어 투 피어 소비.
- 데이터는 그 자신과 가장 가까운 도메인이 제공.
- 데이터를 프로덕트로써 서비스하도록 함.
- 도메인 팀이 이를 할 수 있도록 셀프 서비스 플랫폼 제공.
- 탈중앙화에도 전사 거버넌스(상호 운용성, 컴플라이언스, 보안, 개인정보 보호, 품질, ...)은 충족하도록 함.
