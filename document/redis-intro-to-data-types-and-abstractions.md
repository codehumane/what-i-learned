...
- 실제로는 데이터 타입이 아님.
- 문자열 타입에 대한 비트 지향 연산을 이용한 것.
- 문자열은 바이너리 세이프 BLOB<sup>Binary Large Object</sup>이고,
- 최대 길이는 512MB이므로,
- 2^32개의 서로 다른 비트를 설정하기에 적합.
- 비트 연산은 2개 그룹으로 나뉨.
- 하나는 상수 시간의 단일 비트 연산.
- 예컨대, 한 비트를 1과 0으로 설정하거나 비트의 값 얻기.
- 다른 하나는 비트의 그룹에 대한 연산.
- 예를 들면, 주어진 비트의 범위에 대해 1로 설정된 갯수 얻기.
- 비트 맵의 가장 큰 이점은 정보를 저장할 때의 공간 효율이 극단적으로 좋다는 것.
- 예를 들어, 서로 다른 사용자를 증분 사용자 ID로 표현하는 시스템이 있다고 가정. 이 때 사용자가 4억명이고, 뉴스레터 구독을 신청했는지 여부를 알고자 한다면, 단지 512MB 메모리만 사용하면 됨.
- `SETBIT`와 `GETBIT`를 사용해서 비트를 설정하고 조회.
...
- `SETBIT` 커맨드는 현재 문자열의 길이를 넘어서는 비트 설정일 경우 문자열을 자동으로 확장.
- `GETBIT`는 문자열 길이를 벗어난 범위의 비트에 대해서는 항상 0을 반환.
- 비트의 그룹에 대한 커맨드는 3가지가 존재.
    - [BITOP](https://redis.io/commands/bitop)은 서로 다른 문자열들에 대해 bit-wise(`AND`, `OR`, `XOR`, `NOT`) 연산 수행.
    - [BITCOUNT](https://redis.io/commands/bitcount)는 [population counting](https://en.wikipedia.org/wiki/Population_count) 수행. 즉, 1로 설정된 비트 갯수 반환.
    - [BITOS](https://redis.io/commands/bitpos)는 주어진 값이 나오는 첫 번째 비트의 위치를 반환.
- `BITOP`와 `BITCOUNT`는 전체 문자열 대신 문자열의 일부에 대해서만 연산하는 것도 가능.
- 비트 맵의 사용 사례는 아래와 같음.
    - 모든 종류의 실시간 분석.
    - 객체 ID에 대응하는 boolean 정보를 저장하면서, 공간 효율을 높이고 성능이 좋아야 할 때.
- 웹 사이트 사용자의 매일 방문이 얼마나 길게 연속적으로 이어졌는지를 알아보는 예시 소개.
    - 웹 사이트가 공개된 날짜를 0으로 하여 날짜를 세기 시작.
    - 사용자가 웹 사이트에 방문할 때마다 `SETBIT`로 비트 값을 설정.
    - 비트 인덱스로는 `(현재 유닉스 타임 - 초기 오프셋) / 하루의 초(3600*24)` 사용.
    - 이런 식으로 각 날짜별로 사용자의 방문 정보가 담긴 작은 문자열을 얻게 됨.
    - `BITCOUNT`를 이용하면 주어진 사용자가 웹 사이트에 얼마나 매일 방문했는지를 알 수 있음.
    - 그리고 몇 번의 `BITOS` 호출로 가장 긴 매일 연속 방문 수를 알 수 있음.
- 키 하나가 거대해지는 것을 피하고자 데이터 셋을 샤딩하는 것이 좋음.
- 이를 위해 키 마다 `M`개의 비트를 저장하고,
- 이를 조회할 때는 `bit-number/M`라는 키 이름을 사용하며,
- 키 값 안의 비트를 접근할 때는 `bit-number MOD M`을 이용.

## HyperLogLogs

- 확률적 데이터 구조체.
- 고유한 것의 갯수를 세기 위함.
- 기술적으로는 셋의 카디널리티를 측정하는 것이라 부르기도.
- 일반적으로 아이템을 고유하게 세는 것은, 카운팅 하고 싶은 아이템의 수에 비례하여 메모리 공간이 필요. 이미 과거에 본 원소를 기억하고 있어야 하기 때문.
- HyperLogLogs는 정확성을 다소 포기하는 대신 메모리 공간을 절약할 수 있는 알고리즘.
- 에러 비율은 표준 수치를 넘어가지 않으며, 레디스 구현체에서는 1% 미만.
- 그리고 메모리 공간은 아이템 수에 비례하는 것이 아니라 상수 공간만을 사용.
- 최악의 경우에는 12k 바이트.
- 레디스에서의 HLL은 레디스 문자열로 인코딩 될 수 있음.
- `GET`으로 HLL을 직렬화, `SET`으로 다시 역직렬화.
- 개념적으로 HLL API는 셋의 것과 유사.
- `SADD`를 통해 매 발견된 원소를 셋으로 추가할 수 있고,
- `SCARD`를 통해 셋 안에 있는 원소의 수를 확인.
- HLL 안에 실제로 아이템을 추가하고 싶지 않을 수도 있음.
- 이 때는 [PFADD](https://redis.io/commands/pfadd)와 [PFCOUNT](https://redis.io/commands/pfcount)를 이용.
...
- HLL의 한 가지 사용 예시는 사용자에 의해 수행된 쿼리의 고유 갯수를 세는 것.

## Other notable features

- 이 문서에는 다루지는 않았지만 다른 중요한 기능들이 존재.
- [거대한 컬렉션의 키 스페이스를 하나씩 증가시키면서 순회](https://redis.io/commands/scan)
- [서버 사이드 루아 스크립트 실행](https://redis.io/commands/eval)을 통해 지연과 대역폭 개선.
- [Pub-Sub 서버](https://redis.io/topics/pubsub)로도 사용.
